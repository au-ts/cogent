...
theorem sum_arr_shallow_to_C_refinement:
...
text
  "We can now easily prove the functional correctness of our @{term sum_arr'} C function. In this
   ...
   @{typ \<open>32 word list\<close>} in our shallow embedding. Our functional correctness specification is the
   in-built @{term sum_list} function."

value "sum_list [(0:: 32 word), 1, 2] = Generated_Shallow_Desugar.sum_arr [(0:: 32  word), 1, 2]"

text
  "Before we prove functional correctness, we observe that @{term wordarray_length} returns
   a value of type @{typ \<open>32 word\<close>}. This means that any list that our word array refines in our
   ...
   larger than that would not fit in memory. So we  prove the following:"
...
theorem sum_correct:
  "length xs \<le> unat (max_word :: 32 word)
    \<Longrightarrow> Generated_Shallow_Desugar.sum_arr xs = sum_list xs"
  apply (clarsimp simp: Generated_Shallow_Desugar.sum_arr_def
  ...
  by (subst Groups_List.monoid_add_class.sum_list.eq_foldr)
     (subst foldr_fold, fastforce+)

corollary sum_correct_unat:
  "\<lbrakk>length xs \<le> unat (max_word :: 32 word); sum_list (map unat xs) \<le> unat (max_word :: 32 word)\<rbrakk>
    \<Longrightarrow> unat (Generated_Shallow_Desugar.sum_arr xs) =  (\<Sum>x\<leftarrow>xs. unat x)"
  apply (subst sum_correct; simp?)
  apply (induct xs; clarsimp)
  apply (subst unat_add_simple; simp)
  apply (clarsimp simp: word_bits_def max_word_def)
  done
