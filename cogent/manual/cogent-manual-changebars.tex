...
Block comments may occur in a \cogent source between all other lexical entities. \cbstart Block comments can be nested,
...
\indinfprod{LowercaseID}{
  \cbstart A sequence of letters, digits, underscore symbols, and single quotes\cbend
...
Integer literals are digit sequences. \cbstart They can be written in decimal, hexadecimal. or octal form.
...
\indinfprod{CharacterLiteral}{
  \cbstart A character enclosed in single quotes.\cbend
}
...
\indinfprod{StringLiteral}{
  \cbstart A sequence of characters enclosed in double quotes.\cbend
}
...
types are often automatically inferred and need not be specified explicitly. \cbstart Although possible
in many cases, \cogent never infers types for toplevel definitions (see Section~\ref{toplevel-def}), they
must always be specified explicitly.\cbend
...
\code{\{b: U16, a: U8\}} denote different types. \cbstart A record type must always have atleast one field.\cbend 
...
unlike in C, a variant type is a \textit{discriminated} union: \cbstart each alternative value set is tagged with 
the variant it belongs to. \cbend

A variant type specifies for every variant the tag and the \cbstart types of the values, \cbend hence it has the syntax:
...
\indprod{VariantType}{
  \cbstart \code{<} DataConstructor \{TypeA2\} \{\code{|} DataConstructor \{TypeA2\}\} \code{>}\cbend
}
...
The tags are given by the \textit{DataConstructor} elements. \cbstart Each variant has a sequence of values, hence the 
ordering of the \textit{TypeA2} matters. \cbend
...
\indprod{FunctionType}{
  \cbstart TypeA1 \code{->} TypeA1 \cbend
}

A function with type \code{U8 -> U16} maps values of type \code{U8} to values of type \code{U16}.

Note, that a \textit{TypeA1} \cbdelete cannot be a function type. Hence, to specify a higher order function type in \cogent, which 
...
\cbstart
As in Haskell there is no syntactic difference between type variables and normal (term) variables. \cbend
...
\cbstart The other way round, a type parameter need not occur as type variable in the type expression. In Haskell, this
...
Note that parameterized types must be put in parentheses if they are nested (used as argument of another parameterized type). \cbdelete
...
to know the external C definition for working with an abstract type in \cogent. \cbstart Note that abstract types are not
meant to be used as interfaces to or abstractions of other \cogent types.\cbend
...
\indprod{TakePutFields}{
  ...
  \cbstart\code{(} [FieldName \{\code{,} FieldName\}] \code{)}\cbend
...
\cbstart then applying the take operator is illegal. \cbend
...
\cbstart then applying the put operator is illegal.\cbend
...
\code{Bool}, a \textit{CharacterLiteral} conforms \cbstart only to type \code{U8}.\cbend
...
\indprod{RecordMatchings}{
  \cbstart RecordMatching \{\code{,} RecordMatching\} \cbend
...
\begin{verbatim}
  ...
  #{fld2: U32, fld1: U32}
\end{verbatim}
and all other unboxed record types which have two fields named \code{fld1} and \code{fld2}.\cbend
...
\indprod{VariantPattern}{
  \cbstart DataConstructor \{IrrefutablePattern\}\cbend
}
...
patterns are always refutable. \cbstart As usual, when matched with a value, the match must be exhaustive, specifying
a pattern for every variant. \cbend
...
\cbdelete If an expression has an
...
A term for a value of a variant type specifies the discriminating tag and the actual \cbstart values:\cbend
...
\indprod{VariantTerm}{
  \cbstart DataConstructor \{Term\}\cbend
}
...
\cbstart \code{o}, \code{\&\&}, \code{||} and \code{\$} \cbend which are right associative and \code{<, >, <=, >=, ==, /=} which 
cannot be combined.
...
\cbstart If E is an \textit{IntegerLiteral} of type U \cbend and T is a bitstring type which is a superset of U then 
...
is equivalent to the \textit{MatchingExpression} \cbstart
\begin{tabbing}
  \hspace*{1cm} C \= \code{|} \code{True} \code{->} E \\
                  \> \code{|} \code{False} \code{->} F
\end{tabbing} 
\cbend From this it follows that C must have the inferred type \code{Bool} and E and F must have the same inferred type 
...
resides. \cbstart A \textit{SystemFile}, like in C, specifies a file which is searched at standard places by the
\cogent compiler.\cbend
...
\item the type specification is mandatory and \cbstart in the case of a function type, \cbend instead of a 
...
A \textit{FunctionDefinition} of the form \cbstart
\begin{tabbing}
\hspace*{1cm} \= V \code{:} T \\
              \> V IP \code{=} E
\end{tabbing}
...
Then the \textit{FunctionDefinition} of the form \cbstart
\begin{tabbing}
\hspace*{1cm} \= V \code{:} T\\
              \> V A1 \ldots An
\end{tabbing}
...
\cbstart Function \cbend values bound by toplevel definitions may be \textit{polymorphic} which means that their
...
\indprod{PolyType}{
  ...
  \cbstart \code{all} PermSignatures \code{.} MonoType \cbend
}
...
\indprod{PermSignature}{\cbend
  TypeVariable

  \ldots
}

Here all type variables which occur free in the \textit{MonoType} must be listed in the \cbstart\textit{PermSignatures}.\cbend
...
\end{verbatim}
\cbend
Since the types \cbstart \code{t} and \code{u} \cbend are unknown, no expressions can be specified for their values other than 
variables to which the values have been bound. As a consequence, polymorphic values are \cbstart always \cbend polymorphic functions
which take the values of the unknown types as (part of) their argument and only pass them around, perhaps placing them
in the function result.

A typical example for a \cbstart polymorphic function \cbend works with lists of arbitrary elements.
...
the type is non-linear. Therefore it is possible to specify \cbstart ``permissions'' for a type variable in the
\textit{PermSignatures} \cbend using the following syntax:
...
If no \cbstart \textit{Permissions} are specified for a type variable the default permissions \code{E} apply.\cbend
...
\end{verbatim}
\cbend
the type \code{t} has default \cbstart permissions \cbend \code{E} and is thus required to be escapable. 
...
the free type variables. These types \cbstart can \cbend be explicitly specified using the following syntax:
...
\indprod{PolyVariable}{
  \cbstart Variable \code{[} OptMonoType \{\code{,} OptMonoType\} \code{]}
}
...
\cbstart\code{\{fld1: U8, fld2: U8\}} \cbend which
is not regular.
...
\gramprod{PolyType}{
  ...
  \cbstart\code{all} PermSignatures \code{.} MonoType
}
...
\cbstart\gramprod{IntegerLiteral}{
  DecDigits
...
\graminfprod{OctDigits}{
  A sequence of octal digits 0-7.\cbend
}
