...
definition type_rel_U2_C_def:
  "type_rel typ (_ :: U2_C itself) \<equiv> typ = RCon ''U2'' []"
definition val_rel_U2_C_def:
  "val_rel uv (x :: U2_C) \<equiv> (\<exists>size v. (uv = UAbstract (UUN size v )) \<and>  size = 2 \<and>  v = unat (U2_C.v_C x) \<and>  U2_C.v_C x < 2 ^ size) "
instance ..
end
instantiation U4_C :: cogent_C_val
begin
definition type_rel_U4_C_def:
  "type_rel typ (_ :: U4_C itself) \<equiv> typ = RCon ''U4'' []"
definition val_rel_U4_C_def:
  "val_rel uv (x :: U4_C) \<equiv> (\<exists>size v. (uv = UAbstract (UUN size v )) \<and>  size = 4 \<and>  v = unat (U4_C.v_C x) \<and>  U4_C.v_C x < 2 ^ size) "
instance ..
end
...
local_setup \<open> local_setup_val_rel_type_rel_put_them_in_buckets "main_pp_inferred.c" [UAbstract "U2", UAbstract "U4"] \<close>
...
local_setup \<open> local_setup_heap_rel "main_pp_inferred.c" ["U2_C", "U4_C"] [("32 word", "w32")] \<close>
