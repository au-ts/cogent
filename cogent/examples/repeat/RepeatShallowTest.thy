...
section "@{term repeatatm} Validation"

subsection "Fold Break using @{term repeatatm}"
...
fun my_fold_break :: "('a \<Rightarrow> 'c \<Rightarrow> bool) \<Rightarrow> ('b \<Rightarrow> 'a \<Rightarrow> 'c \<Rightarrow> 'a) \<Rightarrow> 'b list \<Rightarrow> 'a \<Rightarrow> 'c \<Rightarrow> 'a"
  where
"my_fold_break f g xs acc obsv = prod.fst (repeatatm (length xs) (\<lambda>(a, _) (_, b). f a b) (\<lambda>(a, i) (xs, b). (g (xs ! i) a b, Suc i)) (acc, 0) (xs, obsv))"

lemma my_fold_break_helper1:
  "\<lbrakk>n \<le> length xs; f' = (\<lambda>(a, _) (_, b). f a b); g' = (\<lambda>(a, i) (xs, b). (g (xs ! i) a b, Suc i));
    \<not>f' (repeatatm n f' g' (acc, 0) (xs, obsv)) (xs, obsv)\<rbrakk>
    \<Longrightarrow> prod.snd (repeatatm n f' g' (acc, 0) (xs, obsv)) = n"
  apply (induct n)
   apply (simp add: repeatatm.simps)
  apply (clarsimp split: prod.splits)
  apply (insert repeatatm_not_stop_Suc)
  ...
  apply (frule_tac n = n and f = f' and g = g' in repeatatm_step; simp)
  apply (clarsimp split: prod.splits)
  done

lemma my_fold_break_helper1':
  "\<lbrakk>n \<le> length xs;
    \<not>(\<lambda>(a, _) (_, b). f a b) (repeatatm n (\<lambda>(a, _) (_, b). f a b) (\<lambda>(a, i) (xs, b). (g (xs ! i) a b, Suc i)) (acc, 0) (xs, obsv)) (xs, obsv)\<rbrakk>
    \<Longrightarrow> prod.snd (repeatatm n (\<lambda>(a, _) (_, b). f a b) (\<lambda>(a, i) (xs, b). (g (xs ! i) a b, Suc i)) (acc, 0) (xs, obsv)) = n"
  apply (rule my_fold_break_helper1; simp?)
  done

lemma my_fold_break_helper2:
  "\<lbrakk>n \<le> length xs; f' = (\<lambda>(a, _) (_, b). f a b); g' = (\<lambda>(a, i) (xs, b). (g (xs ! i) a b, Suc i));
    repeatatm n f' g' (acc, 0) (xs, obsv) = (x, m);
    \<not>f x obsv\<rbrakk>
    \<Longrightarrow> repeatatm n f' g' (acc, 0) (xs @ ys, obsv) = (x, m)"
  ...
  apply clarsimp
  apply (subst repeatatm_step; clarsimp)
...
lemma my_fold_break_helper2':
  "\<lbrakk>n \<le> length xs;
    repeatatm n  (\<lambda>(a, _) (_, b). f a b) (\<lambda>(a, i) (xs, b). (g (xs ! i) a b, Suc i)) (acc, 0) (xs, obsv) = (x, m);
    \<not>f x obsv\<rbrakk>
    \<Longrightarrow> repeatatm n (\<lambda>(a, _) (_, b). f a b) (\<lambda>(a, i) (xs, b). (g (xs ! i) a b, Suc i)) (acc, 0) (xs @ ys, obsv) = (x, m)"
...
lemma my_fold_break_helper3:
  ...
  apply (induct n arbitrary: x m)
   apply (simp add: repeatatm.simps)
  apply (clarsimp split: prod.splits)
  apply (case_tac "f' (repeatatm n f' g' (acc, 0) (xs, obsv)) (xs, obsv)")
   apply (drule_tac n = n and f = f' and g = g' and a = "(acc, 0)" and b = "(xs, obsv)" in repeatatm_step_stop_Suc; clarsimp)
   apply (drule_tac x = x and y = m in meta_spec2)
   apply (drule meta_mp; simp)
   apply (subst repeatatm_step_stop_Suc; clarsimp split: prod.split)
  ...
  apply (frule_tac n = n and f = f' and g = g' and a = "(acc, 0)" and b = "(xs, obsv)" in repeatatm_step; clarsimp split: prod.split)
  apply (cut_tac f = f and g = g and xs = xs and acc = acc and obsv = obsv and x = x1 and m = n and n = n and ys = ys in  my_fold_break_helper2')
    ...
    apply simp
   apply simp
  apply (subst repeatatm_step; clarsimp split: prod.split)
...
lemma fold_break_my_fold_break_eq:
  ...
  apply (induct rule: rev_induct[where xs = xs])
   apply (simp add: repeatatm.simps)
  apply clarsimp
  apply (cut_tac p = "repeatatm (length xs) (\<lambda>(a, _) (_, y). f a y) (\<lambda>(a, i) (xs, b). (g (xs ! i) a b, Suc i)) (acc, 0) (xs, obsv)" in surj_pair)
  apply (erule exE)+
  ...
  apply (rule conjI; rule impI)
   apply (frule_tac n = "length xs" and xs = xs and x = y and ys = "[x]" and m = m in my_fold_break_helper3'[rotated 1])
    ...
    apply simp
   apply simp
   apply (subst repeatatm_step_stop_Suc; clarsimp split: prod.splits)
  ...
  apply (cut_tac n = "length xs" and xs = xs and f = f and g = g and acc = acc and obsv = obsv in my_fold_break_helper1'[rotated 1])
    apply simp
   apply simp
  apply (subst repeatatm_step; clarsimp split: prod.splits)
  done
