...
(*
thm repeat_0'_def[simplified L2polish]
definition repeat_c
  where
"repeat_c a \<equiv>
    do ret_val <- select UNIV;
       (i :: 64 word) <- gets (\<lambda>_. 0);
       b <- gets (\<lambda>_. t2_C.obsv_C_update (\<lambda>_. t5_C.obsv_C a) (t2_C.acc_C_update (\<lambda>_. t5_C.acc_C a) (t2_C (t1_C 0 0) 0)));
       c <- gets (\<lambda>_. t2_C.obsv_C_update (\<lambda>_. t5_C.obsv_C a) (t2_C.acc_C_update (\<lambda>_. t5_C.acc_C a) (t2_C (t1_C 0 0) 0)));
       i <- gets (\<lambda>_. 0);
       doE c <-
           doE x <-
               doE _ <- liftE (guard (\<lambda>s. True));
                   whileLoopE (\<lambda>(ret, b, c, i) s. i < n_C a)
                       (\<lambda>(ret, b, c, i).
                           doE x <-
                               doE retval <- liftE (do ret' <- dispatch_t3' (stop_C a) b; gets (\<lambda>_. ret') od);
                                   _ <- 
                                   doE _ <- liftE (guard (\<lambda>s. True));
                                       condition (\<lambda>s. boolean_C retval \<noteq> 0)
                                           (doE global_exn_var <- liftE (gets (\<lambda>_. Break));
                                               throwError (global_exn_var, ret, c)
                                            odE)
                                           (liftE (gets (\<lambda>_. ())))
                                   odE;
                                   liftE
                                       (do retval <- do ret' <- dispatch_t4' (step_C a) c; gets (\<lambda>_. ret') od;
                                           c <- gets (\<lambda>_. t2_C.acc_C_update (\<lambda>_. retval) c);
                                           b <- gets (\<lambda>_. t2_C.acc_C_update (\<lambda>_. t2_C.acc_C c) b);
                                           i <- gets (\<lambda>_. i + 1);
                                           gets (\<lambda>_. (retval, b, c, i))
                                        od)
                               odE;
                               liftE (case x of (ret, b, c, i) \<Rightarrow> do _ <- guard (\<lambda>_. True); gets (\<lambda>_. (ret, b, c, i)) od)
                           odE)
                       (ret_val, b, c, i)
               odE;
               liftE (case x of (ret, b, c, i) \<Rightarrow> gets (\<lambda>_. c))
           odE <handle2>
           (\<lambda>(global_exn_var, ret, c). 
               doE _ <-
                   doE _ <- liftE (guard (\<lambda>s. True));
                      condition (\<lambda>s. global_exn_var = Break)
                          ...
                          (throwError ret)
                   odE;
                   liftE (gets (\<lambda>_. c))
               odE);
           ret <- liftE (gets (\<lambda>_. t2_C.acc_C c));
           global_exn_var <- liftE (gets (\<lambda>_. Return));
           throwError ret
       odE <catch>
       (\<lambda>ret. do _ <- gets (\<lambda>_. ()); gets (\<lambda>_. ret) od)
    od"
