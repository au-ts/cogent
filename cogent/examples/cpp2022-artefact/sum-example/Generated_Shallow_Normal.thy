...
definition
  wordarray_get_u32 :: "(32 word WordArray,  WordArrayIndex) T0 \<Rightarrow> 32 word"
where
  "wordarray_get_u32 ds\<^sub>0 \<equiv> HOL.Let ds\<^sub>0 (\<lambda>x. (wordarray_get :: (32 word WordArray,  WordArrayIndex) T0 \<Rightarrow> 32 word) x)"
...
where
  "wordarray_put2_u32 ds\<^sub>0 \<equiv> HOL.Let ds\<^sub>0 (\<lambda>x. (wordarray_put2 :: 32 word WordArrayPutP\<^sub>T \<Rightarrow> 32 word WordArray) x)"
...
where
  "sum_arr ds\<^sub>0 \<equiv> HOL.Let ds\<^sub>0 (\<lambda>wa. HOL.Let ((wordarray_length :: 32 word WordArray \<Rightarrow> 32 word) wa) (\<lambda>e. HOL.Let (0 :: 32 word) (\<lambda>an\<^sub>1\<^sub>0. HOL.Let add (\<lambda>an\<^sub>1\<^sub>2. HOL.Let (0 :: 32 word) (\<lambda>an\<^sub>1\<^sub>3. HOL.Let () (\<lambda>an\<^sub>1\<^sub>4. HOL.Let (WordArrayMapNoBreakP.make wa an\<^sub>1\<^sub>0 e an\<^sub>1\<^sub>2 an\<^sub>1\<^sub>3 an\<^sub>1\<^sub>4) (\<lambda>arg. (wordarray_fold_no_break :: (32 word, 32 word, unit) WordArrayFoldNoBreakP\<^sub>T \<Rightarrow> 32 word) arg)))))))"

definition
  dec :: "(32 word, unit, unit) ElemAO\<^sub>T \<Rightarrow> (32 word, unit) T0"
where
...
where
  "dec_arr ds\<^sub>0 \<equiv> HOL.Let ds\<^sub>0 (\<lambda>wa. HOL.Let ((wordarray_length :: 32 word WordArray \<Rightarrow> 32 word) wa) (\<lambda>end. HOL.Let (0 :: 32 word) (\<lambda>an\<^sub>2\<^sub>5. HOL.Let dec (\<lambda>an\<^sub>2\<^sub>7. HOL.Let () (\<lambda>an\<^sub>2\<^sub>8. HOL.Let () (\<lambda>an\<^sub>2\<^sub>9. HOL.Let (WordArrayMapNoBreakP.make wa an\<^sub>2\<^sub>5 end an\<^sub>2\<^sub>7 an\<^sub>2\<^sub>8 an\<^sub>2\<^sub>9) (\<lambda>arg. (wordarray_map_no_break :: (32 word, unit, unit) WordArrayMapNoBreakP\<^sub>T \<Rightarrow> (32 word WordArray, unit) T0) arg)))))))"

definition
  inc :: "(32 word, unit, unit) ElemAO\<^sub>T \<Rightarrow> (32 word, unit) T0"
where
...
where
  "inc_arr ds\<^sub>0 \<equiv> HOL.Let ds\<^sub>0 (\<lambda>wa. HOL.Let ((wordarray_length :: 32 word WordArray \<Rightarrow> 32 word) wa) (\<lambda>end. HOL.Let (0 :: 32 word) (\<lambda>an\<^sub>4\<^sub>0. HOL.Let inc (\<lambda>an\<^sub>4\<^sub>2. HOL.Let () (\<lambda>an\<^sub>4\<^sub>3. HOL.Let () (\<lambda>an\<^sub>4\<^sub>4. HOL.Let (WordArrayMapNoBreakP.make wa an\<^sub>4\<^sub>0 end an\<^sub>4\<^sub>2 an\<^sub>4\<^sub>3 an\<^sub>4\<^sub>4) (\<lambda>arg. (wordarray_map_no_break :: (32 word, unit, unit) WordArrayMapNoBreakP\<^sub>T \<Rightarrow> (32 word WordArray, unit) T0) arg)))))))"
...
where
  "mul_arr ds\<^sub>0 \<equiv> HOL.Let ds\<^sub>0 (\<lambda>wa. HOL.Let ((wordarray_length :: 32 word WordArray \<Rightarrow> 32 word) wa) (\<lambda>e. HOL.Let (0 :: 32 word) (\<lambda>an\<^sub>5\<^sub>3. HOL.Let mul (\<lambda>an\<^sub>5\<^sub>5. HOL.Let (0 :: 32 word) (\<lambda>an\<^sub>5\<^sub>6. HOL.Let () (\<lambda>an\<^sub>5\<^sub>7. HOL.Let (WordArrayMapNoBreakP.make wa an\<^sub>5\<^sub>3 e an\<^sub>5\<^sub>5 an\<^sub>5\<^sub>6 an\<^sub>5\<^sub>7) (\<lambda>arg. (wordarray_fold_no_break :: (32 word, 32 word, unit) WordArrayFoldNoBreakP\<^sub>T \<Rightarrow> 32 word) arg)))))))"
