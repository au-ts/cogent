...
text 
  "@{term wordarray_length} returns the length of a word array.
   ...
   Below is a proof tactic to prove @{term scorres} for wordarray_length. The proof is
   quite simple as @{term scorres} can be proven by simplifying the @{term valRel} definitions
...
let  
  ...
  val base_simpset = @{thms scorres_def valRel_records wordarray_length' valRel_WordArray_simps};
in
...
(*lemma scorres_wordarray_length_u8:
  "\<lbrakk>valRel \<xi>p (v:: 8 word WordArray) v'\<rbrakk>
    \<Longrightarrow> scorres (wordarray_length v) (App (AFun ''wordarray_length'' ts) (Var 0)) [v'] \<xi>p"
  by (tactic \<open>wa_length_tac @{context}\<close>)
...
text 
  "@{term wordarray_get} is similar to the @{term nth} function for lists, except is does not cause
   ...
   Below is a proof tactic to prove @{term scorres} for wordarray_get. The proof is
...
let  
  ...
  fun clarsimp_split thm i = Clasimp.clarsimp_tac (Splitter.add_split thm ctxt) i 
  val base_simpset = @{thms scorres_def valRel_records wordarray_get' valRel_WordArray_simps};
in
  clarsimp_add base_simpset 1
  THEN clarsimp_split @{thm if_split_asm} 1
  THEN ALLGOALS (fn a =>
...
text
  "Below is a proof tactic to prove @{term scorres} for wordarray_put2. The proof tactic was
...
let  
  fun clarsimp_add thms = Clasimp.clarsimp_tac (add_simps thms ctxt);
  fun clarsimp_split thm i = Clasimp.clarsimp_tac (Splitter.add_split thm ctxt) i
  val base_simpset = @{thms scorres_def valRel_records wordarray_put2' valRel_WordArray_simps};
...
text 
  \<open> We cannot generalise this unless you can prove that 
    @{term "\<forall>a b. valRel \<xi>\<^sub>i (a :: 'a) b \<longleftrightarrow> valRel \<xi>\<^sub>j a b"}
    which is generally not true for functions.\<close>
