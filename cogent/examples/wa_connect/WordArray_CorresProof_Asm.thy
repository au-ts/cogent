...
lemma upd_wa_foldnb_preservation:
  ...
  apply (frule_tac v = obsv and K = "[]" and k = "kinding_fn [] \<tau>b" in upd.discardable_or_shareable_not_writeable(1)[rotated 1]; simp?)
...
lemma upd_val_wa_foldnb_bod_corres:
  "\<lbrakk>proc_ctx_wellformed \<Xi>'; proc_env_u_v_matches \<xi>\<^sub>u \<xi>\<^sub>v \<Xi>'; upd.proc_env_matches_ptrs \<xi>\<^sub>u \<Xi>';
  ...
  apply (frule_tac \<gamma> = "[URecord [(va, RPrim (Num ta)), (resa, type_repr u), 
   ...
   apply (rule u_v_matches_some[where r' = "{}" and w' = "{}", simplified])
    ...
    apply (rule u_v_r_cons1[where r = "{}" and w = "{}", simplified]; simp?)
     apply (drule abs_upd_val_frame; simp?)
...
lemma upd_val_wa_mapAccumnb_bod_corres:
  "\<lbrakk>proc_ctx_wellformed \<Xi>'; proc_env_u_v_matches \<xi>\<^sub>u \<xi>\<^sub>v \<Xi>'; upd.proc_env_matches_ptrs \<xi>\<^sub>u \<Xi>';
    ...
    wa_abs_upd_val (UWA t len arr) (VWA t xs) ''WordArray'' \<tau>s (Boxed Writable ptrl) ra wa \<sigma>;
    ...
    \<Longrightarrow>  \<exists>rp ta uacc r' w' xs' vacc. 
      res = URecord [(rp, upd.uval_repr rp), (uacc, type_repr u)]
    \<and> res' = VRecord[VAbstract (VWA t xs'), vacc]
    \<and> rp = UPtr p (RCon ''WordArray'' [type_repr t])
    \<and> t = TPrim (Num ta)
    \<and> wa_abs_upd_val (UWA t len arr) (VWA t xs') ''WordArray'' \<tau>s (Boxed Writable ptrl) ra wa \<sigma>'
  ...
  apply (induct to arbitrary:  \<sigma>' res' res)
   ...
   apply (clarsimp simp: upd.frame_id Int_commute)
   ...
   apply (rule u_v_matches_some[where r' = "{}" and w' = "{}", simplified])
    ...
    apply (rule u_v_r_cons1[where r' = rc and w' = "{}", simplified]; simp?)
     apply (rule u_v_r_cons1[where r' = "{}" and w' = "{}", simplified]; simp?)
      ...
      apply (rule upd.subset_helper[OF _ subset_refl
  ...
  apply (rule conjI)
   apply (drule_tac \<sigma> = \<sigma>'' and ?w1.0 = w' in  abs_upd_val_frame; simp?)
  ...
  apply (drule_tac s = "{arr + size_of_num_type ta * i |i. i < len \<and> frm \<le> i \<and> i < 1 + to}" and
   ...
   apply (drule_tac ?w1.0 = w' and ?w2.0 = w'' and \<sigma> = \<sigma>'' in abs_upd_val_frame; simp?)
...
lemma wordarray_fold_no_break_upd_val:
  ...
  apply clarsimp
  apply (erule u_v_r_consE'; clarsimp)
  apply (frule upd.tfun_no_pointers(1)[OF upd_val_rel_to_uval_typing(1)]; clarsimp)
  apply (frule upd.tfun_no_pointers(2)[OF upd_val_rel_to_uval_typing(1)]; clarsimp)
  apply (erule u_v_r_consE'; clarsimp)+
  apply (erule u_v_r_emptyE'; clarsimp)
  ...
  apply (frule_tac v = uobsv and K = "[]" and k = "kinding_fn [] \<tau>o" 
      in upd.discardable_or_shareable_not_writeable(1)[rotated 1, OF upd_val_rel_to_uval_typing(1)]; simp?)
    apply (rule kindingI; clarsimp)
...
lemma upd_wa_mapAccumnb_preservation:
  ...
  apply (subst (asm) \<Xi>_def; simp add: wordarray_map_no_break_0_type_def abbreviated_type_defs)
  apply (frule_tac v = obsv and K = "[]" and k = "kinding_fn [] \<tau>o" in upd.discardable_or_shareable_not_writeable(1)[rotated 1]; simp?)
   apply (rule kindingI; clarsimp)
  apply clarsimp
  ...
  apply (erule impE)
   apply (rule upd.u_t_struct)
    apply (rule upd.u_t_r_cons1; simp?)
      apply (rule upd.u_t_p_abs_w[where ts = "[TPrim (Num _)]", simplified]; simp?)
      apply (drule wa_abs_typing_u_elims(3))
      apply (drule_tac p = p in valid_ptr_not_in_frame_same; simp?)
      apply clarsimp
     apply (rule upd.u_t_r_cons1[where r' = "{}" and w' = "{}" and xs = "[]" and ts = "[]", simplified]; simp?)
...
lemma wordarray_mapAccumnb_abs_upd_val:
  ...
  apply clarsimp
  apply (erule u_v_p_absE'; clarsimp)
  apply (frule upd.tfun_no_pointers(1)[OF upd_val_rel_to_uval_typing(1)]; simp)
  apply (frule upd.tfun_no_pointers(2)[OF upd_val_rel_to_uval_typing(1)]; simp)
  apply clarsimp
  apply (rename_tac ra f f' acc acc' rb wb obsv obsv' rc wc a' wa)
  apply (subst (asm) \<Xi>_def; simp add: wordarray_map_no_break_0_type_def abbreviated_type_defs)
  apply (frule_tac v = obsv and K = "[]" and k = "kinding_fn [] \<tau>o"
      in upd.discardable_or_shareable_not_writeable(1)[rotated 1, OF upd_val_rel_to_uval_typing(1)];
      simp?)
   apply (rule kindingI; clarsimp)
  apply (subst (asm) \<Xi>_def; simp add: wordarray_map_no_break_0_type_def abbreviated_type_defs)
