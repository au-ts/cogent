...
deepExpr mod ta defs (TE _ (Variable v loc)) = mkApp (mkId "Var") [deepIndex (fst v)]
deepExpr mod ta defs (TE _ (Fun fn ts ls _ _))  -- FIXME
...
deepExpr mod ta defs (TE (TSum alts) (Con cn e _ _))
...
deepExpr mod ta defs (TE _ (Esac e@(TE (TSum alts) _) _))
...
deepExpr mod ta defs (TE _ (Take _ rec fld e _))
