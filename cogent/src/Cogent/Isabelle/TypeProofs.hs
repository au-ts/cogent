...
splitEnv env (TE t (Cast ty e loc))
    = let e' = splitEnv env e
       in EE t (Cast ty e' loc) $ envOf e'
...
splitEnv env (TE t (Struct fs loc))
    = let fs' = map (splitEnv env . snd) fs
       in EE t (Struct (zip (map fst fs) fs') loc) $ foldl (<|>) (cleared env) (map envOf fs')
...
splitEnv env (TE t (Take a e f e2 loc)) =
    let e' = splitEnv env e
     ...
     in EE t (Take a e' f e2' loc) $ envOf e' <|> peel2 (envOf e2')
...
pushDown unused (EE ty (Struct fs loc) env)
    = let fs' = case map snd fs of
       ...
       in EE ty (Struct (zip (map fst fs) fs') loc) $ unused <|> env
...
pushDown unused (EE ty (Take a e f e2 loc) env)
...
pushDown unused (EE ty (Cast ty' e loc) env)
    = let e' = pushDown unused e
       in EE ty (Cast ty' e' loc) $ unused <|> env
...
typeTree (EE ty (Take a e1 f e2 _) env) = TyTrSplit (treeSplits env (envOf e1) (peel2 $ envOf e2)) ([], typeTree e1) ([envOf e2 `at` FZero, envOf e2 `at` FSuc FZero], typeTree e2)
