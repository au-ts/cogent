...
import qualified Cogent.TypeCheck as T
import Cogent.Util
...
type Constants  = M.Map VarName  T.TypedExpr  -- This shares namespace with `TermVars'
type Enumerator = Int
...
desugarAlts e0@(T.TE t v@(S.Var _)) ((S.Alt p1 l1 e1):alts) =  -- More than one Alt, which means the pattern cannot be IrrefPattern
  case p1 of
    ...
    S.PCon cn1 [p1'] -> do  -- This is B) for PCon
      ...
      let p1'' = S.PVar (v1,t1)
          ...
          e1' = T.TE (T.typeOfTE e1) $ S.Let [b] e1
      desugarAlts e0 ((S.Alt (S.PCon cn1 [p1'']) l1 e1'):alts)
    S.PCon cn1 ps -> do  -- This is C) for PCon
      t' <- typeWHNF t
      desugarAlts (T.TE t' v) ((S.Alt (S.PCon cn1 [S.PTuple ps]) l1 e1):alts)
    S.PIntLit  i -> desugarPrimInt <$> typeWHNF (T.typeOfTE e0) >>= \pt ->
...
desugarAlts e0 alts@((S.Alt _ _ e1):_) = do  -- e0 is not a var, so lift it
  v <- freshVar
  let t0 = T.typeOfTE e0
      t1 = T.typeOfTE e1
      b = S.Binding (S.PVar (v,t0)) Nothing e0 []
      m = T.TE t1 $ S.Match (T.TE t0 $ S.Var v) [] alts
...
desugarAlt e0 (S.PCon tag [p]) e = do  -- Ind. step A)
  ...
  -- desugarExpr $ T.TE (T.typeOfTE e) $ S.Let [b0,b1] e
  let e' = T.TE (T.typeOfTE e) $ S.Let [b1] e
  desugarAlt e0 (S.PCon tag [S.PVar (v,t)]) e'
desugarAlt (T.TE t e0) (S.PCon tag []) e = do  -- Ind. B1)
  t' <- typeWHNF t
  desugarAlt (T.TE t' e0) (S.PCon tag [S.PUnitel]) e
desugarAlt (T.TE t e0) (S.PCon tag ps) e = do  -- B2)
  t' <- typeWHNF t
  -- FIXME: zilinc
...
desugarAlt e0 (S.PIrrefutable (S.PTuple [S.PVar tn1, S.PVar tn2])) e | not __cogent_ftuples_as_sugar =
  ...
  --   XXX | let S.RT (S.TTuple ts) = T.typeOfTE e0
  --   XXX |     pvs = P.zipWith (curry $ S.PVar) vns ts
  --   XXX |     vs  = P.zipWith (\t v -> T.TE t $ S.Var v) ts vns
  --   XXX |     b0 = S.Binding (S.PTuple pvs) Nothing e0 []
  --   XXX |     bs = P.zipWith (\p v -> S.Binding p Nothing v []) ps vs
  --   XXX | desugarExpr (T.TE (T.typeOfTE e) $ S.Let (b0:bs) e)
  -- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
desugarAlt e0 (S.PIrrefutable (S.PTuple [p1,p2])) e | not __cogent_ftuples_as_sugar = do
  ...
  desugarExpr $ T.TE (T.typeOfTE e) $ S.Let [b0,b1,b2] e  -- Mutual recursion here
...
desugarAlt e0 (S.PIrrefutable (S.PTuple ps)) e | __cogent_ftuples_as_sugar, and (P.map isPVar ps) = do
  ...
  where isPVar (S.PVar _) = True; isPVar _ = False
        ...
        mkTake :: UntypedExpr t v VarName -> [VarName] -> T.TypedExpr -> Int -> DS t v (UntypedExpr t v VarName)
...
desugarAlt e0 (S.PIrrefutable (S.PSequence (p:ps))) e = do
  ...
  vs <- freshVar
  e0' <- desugarExpr e0
  let S.RT (S.TSequence (Just (le,te))) = T.typeOfTE e
  e' <- withBindings (Cons v1 (Cons vs Nil)) $
          desugarAlt (T.TE (S.RT $ S.TSequence $ Just (le-1,te)) $ S.Var vs) (S.PIrrefutable (S.PSequence ps)) e
  return . E $ UnSeqCons (v1,vs) e0' e'
...
desugarAlt e0 (S.PIrrefutable (S.PTake rec [Just (f, S.PVar v)])) e =
  ...
  desugarType (T.typeOfTE e0) >>= \(TRecord fs _) -> let Just fldIdx = elemIndex f (P.map fst fs) in
...
typeWHNF (S.RT (S.TBang t)) = T.bangType <$> typeWHNF t
...
desugarExpr (T.TE _ (S.PrimOp Syn.Cons [e1,e2])) = freshVar >>= \v -> E <$> (SeqConsC v <$> desugarExpr e1 <*> desugarExpr e2 <*> pure (E $ Variable (f0, v)))
...
desugarExpr (T.TE _ (S.Var vn)) = (findIx vn . sel2 <$> get) >>= \case
  ...
  Nothing -> do constdefs <- view _2
                ...
                desugarExpr e
desugarExpr (T.TE _ (S.Match e _ [])) = __impossible "desugarExpr (Match)"
desugarExpr (T.TE _ (S.Match e [] alts)) = desugarAlts e alts
desugarExpr (T.TE _ (S.Match e vs alts)) = do
  ...
  e' <- withBinding v $ desugarAlts (T.TE (T.typeOfTE e) $ S.Var v) alts
  E <$> (LetBang vs' v <$> desugarExpr e <*> pure e')
desugarExpr (T.TE _ (S.TypeApp v ts note)) = do
  pragmas <- view _3
...
desugarExpr (T.TE _ (S.App (T.TE _ (S.TypeApp ('_':_) _ _)) _)) | not __cogent_debug = return (E Unit)
desugarExpr (T.TE _ (S.App e1 e2)) = E <$> (App <$> desugarExpr e1 <*> desugarExpr e2)
desugarExpr (T.TE _ (S.If c [] th el)) = E <$> (If <$> desugarExpr c <*> desugarExpr th <*> desugarExpr el)
desugarExpr (T.TE _ (S.If c vs th el)) = do
  venv <- use _2
  v <- freshVar
  let vs' = P.map (fromJust . flip findIx venv &&& id) vs
...
desugarExpr (T.TE t (S.IntLit n)) = return $ E . ILit n $ desugarPrimInt t
desugarExpr (T.TE _ (S.BoolLit b)) = return $ E $ ILit (if b then 1 else 0) Boolean
desugarExpr (T.TE _ (S.CharLit c)) = return $ E $ ILit (fromIntegral $ ord c) U8
desugarExpr (T.TE _ (S.StringLit s)) = return $ E $ SLit s
desugarExpr (T.TE _ (S.Tuple [])) = __impossible "desugarExpr (Tuple)"
desugarExpr (T.TE _ (S.Tuple [e])) = __impossible "desugarExpr (Tuple)"
desugarExpr (T.TE _ (S.Tuple (e1:e2:[]))) | not __cogent_ftuples_as_sugar = E <$> (Tuple <$> desugarExpr e1 <*> desugarExpr e2)
desugarExpr (T.TE t (S.Tuple (e1:e2:es))) | not __cogent_ftuples_as_sugar = __impossible "desugarExpr"  -- do
  -- S.RT (S.TTuple (t1:t2:ts)) <- typeWHNF t
  -- let t2' = S.RT $ S.TTuple (t2:ts)
  --     e2' = T.TE t2' $ S.Tuple (e2:es)
...
desugarExpr (T.TE _ (S.Let [S.Binding (S.PVar v) mt e0 bs] e)) = do
  -- Idea:
...
desugarExpr (T.TE t (S.Let [S.Binding p mt e0 bs] e)) = do
  v <- freshVar
  let t0 = T.typeOfTE e0
      b0 = S.Binding (S.PVar (v,t0)) Nothing e0 bs
      b1 = S.Binding p mt (T.TE t0 $ S.Var v) []
  desugarExpr (T.TE t $ S.Let [b0,b1] e)
desugarExpr (T.TE t (S.Let (b:bs) e)) = desugarExpr $ T.TE t (S.Let [b] e')
  where e' = T.TE t $ S.Let bs e
