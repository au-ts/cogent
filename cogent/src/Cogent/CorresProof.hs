...
setupFunctionValRel thy =
  ...
  , "       then [@{mk_term \"x = sint (?tag :: 32 signed word) \\<and> uv = UFunction (?fun :: string expr) []\""
  , "               (tag, fun)} (tag_term, Syntax.read_term @{context} (\"" ++ thy ++ "_TypeProof.\" ^ fun_name))]"
  , "       else [@{mk_term \"x = sint (?tag :: 32 signed word) \\<and> uv = UAFunction (?fun :: string) []\""
  ...
  , "|> (fn def => @{mk_term \"cogent_function_val_rel uv x \\<equiv> ?def\" def} def)"
  , "|> strip_type |> Syntax.check_term @{context}"
  , "|> (fn def => Global_Theory.add_defs true [((Binding.name \"cogent_function_val_rel\", def), [])] #> prod.snd)"
  , "end"
  , "*}"
  ]
...
nameField =
  ...
  , "   fun typ_string T = dest_Type T |> prod.fst"
...
checkHofHints =
  ...
  , "                  else error (\"HO_call_hints: absfun \" ^ quote af ^ \" not in \" ^ quote f)) (map prod.fst calls)"
...
xiN =
  ...
  , "val (deepest_tree::_) ="
  , "  Symtab.dest Cogent_main_tree |> map prod.snd |> filter (fn tr =>"
  , "    CogentCallTree_data tr ="
  , "    (Symtab.dest Cogent_main_tree |> map prod.snd |> map CogentCallTree_data |> maximum))"
  , "*}"
