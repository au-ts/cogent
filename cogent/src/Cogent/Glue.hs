...
coreTcAnti m a = view kenv >>= \(fmap snd -> ts) -> lift . lift $
  StateT $ \s -> let reader = (ts, view (coreTcState.funtypes) s)
                  in case flip evalStateT (Nil, [], 0) $ flip runReaderT reader $ runExceptT $ IN.unTC $ m a of
                  -- in case flip runReaderT reader $ runExceptT $ IN.unTC $ m a of
                  -- in case runExceptT $ IN.unTC $ m a of
...
tcType t = do
  ...
  flip tcAnti t $ \t -> do TC.errCtx %= (TC.AntiquotedType t :)
                           ...
                           ((ct,t'),flx,os,fvar) <- TC.runCG ctx tvs [] $ TC.validateType $ SF.stripLocT t
                           (gs, subst) <- TC.runSolver (TC.solve (L.zip tvs $ repeat k2) [] $ ct) flx fvar
...
tcExp e mt = do
  ...
  flip tcAnti e $ \e ->
    do let ?loc = SF.posOfE e
       ...
       ((c,e'),flx,os,fvar) <- TC.runCG ctx (L.map fst vs) [] (TC.cg e =<< maybe TC.freshTVar return mt)
       (cs, subst) <- TC.runSolver (TC.solve vs [] c) flx fvar
