...
newtype Mono b x = Mono { runMono :: RWS (Instance b)
                                         ([Warning], [Definition PosTypedExpr (VarName, Maybe VarName) b], [(Type 'Zero b, String)], [Pragma_ b])
                                         (FunMono b, InstMono b)
                                         x }
                 deriving (Functor, Applicative, Monad,
                           MonadReader (Instance b),
                           MonadWriter ([Warning], [Definition PosTypedExpr (VarName, Maybe VarName) b], [(Type 'Zero b, String)], [Pragma_ b]),
                           MonadState  (FunMono b, InstMono b))

-- Returns: (monomorphic abstract functions, poly-abs-funcs)
absFuns :: [Definition PosTypedExpr (VarName, Maybe VarName) b] -> ([FunName], [FunName])
...
absFunMono :: FunMono b -> [Definition PosTypedExpr (VarName, Maybe VarName) b] -> M.Map FunName Int
...
printAFM :: FunMono b -> [Definition PosTypedExpr (VarName, Maybe VarName) b] -> String
printAFM = ((unlines . P.map (\(n,i) -> n ++ ", " ++ show i) . M.toList) .) . absFunMono

mono :: forall b. (Ord b)
     => [Definition PosTypedExpr (VarName, Maybe VarName) b]
     -> [(SupposedlyMonoType b, String)]
     -> [Pragma b]
     -> Maybe (FunMono b, InstMono b)
     -> ((FunMono b, InstMono b), ([Warning], [Definition PosTypedExpr (VarName, Maybe VarName) b], [(Type 'Zero b, String)], [Pragma_ b]))
...
monoDefinitions :: (Ord b) => [Definition PosTypedExpr (VarName, Maybe VarName) b] -> Mono b ()
monoDefinitions = mapM_ monoDefinition

monoDefinition :: (Ord b) => Definition PosTypedExpr (VarName, Maybe VarName) b -> Mono b ()
...
monoDefinitionInsts :: (Ord b) => Definition PosTypedExpr (VarName, Maybe VarName) b -> [Instance b] -> Mono b ()
...
-- given one instance
monoDefinitionInst :: (Ord b) => Definition PosTypedExpr (VarName, Maybe VarName) b -> Instance b -> Mono b ()
...
getPrimInt :: PosTypedExpr t v (VarName, Maybe VarName) b -> PrimInt
getPrimInt (TE t _) | TPrim p <- t = p
                    | otherwise = __impossible "getPrimInt"

monoExpr :: (Ord b) => PosTypedExpr t v (VarName, Maybe VarName) b -> Mono b (PosTypedExpr 'Zero v (VarName, Maybe VarName) b)
monoExpr (TE t e) = TE <$> monoType t <*> monoExpr' e
  where
    ...
    monoExpr' (SLit    s loc       ) = pure $ SLit s loc
    ...
    monoExpr' (Esac    e loc          ) = Esac <$> monoExpr e <*> pure loc
    monoExpr' (Split   a tp e loc     ) = Split a <$> monoExpr tp <*> monoExpr e <*> pure loc
