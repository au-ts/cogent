...
typeProof thy = TheoryString $ unlines
  ...
  , "    in Local_Theory.note ((Binding.name (f ^ \"_typecorrect'\"), []), [t_thm]) ctxt |> prod.snd"
  , "    end)"
...
exportThms thy = TheoryString $ unlines
  ...
  , "  |> filter (member (op=) entry_func_names o #1 o prod.snd)"
  , "  |> filter (member (op=) Cogent_functions o #1 o prod.snd)"
  , "  |> sort_wrt (#1 o prod.snd)"
  , "  |> partition_eq (fn (p1,p2) => #1 (prod.snd p1) = #1 (prod.snd p2))"
  , "  |> map (sort (option_ord int_ord o"
  , "                both (fn p => unprefix (#1 (prod.snd p) ^ \"_corres_\") (fst p)"
  , "                              |> Int.fromString))"
  , "          #> List.last)"
  , "*}"
  , "local_setup {*"
  , "fold (fn (f, p) => Utils.define_lemmas (\"corres_\" ^ #1 p)"
  , "                     [Symtab.lookup finalised_thms f |> the |> the] #> prod.snd)"
  ...
  , "local_setup {*"
  , "fold (fn (f, thm) => Utils.define_lemmas (f ^ \"_monomorphic\") [thm] #> prod.snd)"
  ...
  , "local_setup {*"
  , "fold (fn (f, thm) => Utils.define_lemmas (f ^ \"_normalised\") [thm] #> prod.snd)"
