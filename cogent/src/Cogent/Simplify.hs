...
markOcc sv (TE tau (Op opr es loc)) = TE tau . flip (Op opr) loc <$> mapM (markOcc sv) es
...
markOcc sv (TE tau (If c th el loc)) = do
  ...
  return $ TE tau $ If c' th' el' loc
markOcc sv (TE tau (Case e tag (l1,a1,e1) (l2,a2,e2) loc)) = do
  e' <- markOcc sv e
...
simplExpr sv subst ins (TE tau (Op opr es loc)) cont = TE tau . flip (Op opr) loc <$> mapM (flip (simplExpr sv subst ins) cont) es
simplExpr sv subst ins (TE tau (App (TE tau1 (Fun fn tys lvs note locFun)) e2 locApp)) cont
  ...
  case def of
    ...
    AbsDecl attr fn ts ls ti to    -> return $ TE tau $ App (TE tau1 (Fun (unsafeCoreFunName fn) tys lvs note locFun)) e2' locApp  -- FIXME
...
simplExpr sv subst ins (TE tau (If c th el loc)) cont = do
  ...
  return $ TE tau $ If c' th' el' loc
simplExpr sv subst ins (TE tau (Case e tn (l1,n1,e1) (l2,n2,e2) loc)) cont = do  -- FIXME
...
noLinear (TE tau e) = (&&) <$> typeNotLinear tau <*> noLinear' e
  where
    ...
    noLinear' (Op _ es _) = and <$> mapM noLinear es
    noLinear' (App e1 e2 _) = (&&) <$> noLinear e1 <*> noLinear e2
    noLinear' (Con _ e _ _) = noLinear e
    ...
    noLinear' (Cast ty e _) = noLinear e
...
lowerExpr w i (TE tau (Op opr es loc))          = TE tau $ Op opr (L.map (lowerExpr w i) es) loc
...
lowerExpr w i (TE tau (Cast ty e loc))          = TE tau $ Cast ty (lowerExpr w i e) loc
...
liftExpr i (TE tau (Op opr es loc))          = TE tau $ Op opr (L.map (liftExpr i) es) loc
...
liftExpr i (TE tau (Cast ty e loc))          = TE tau $ Cast ty (liftExpr i e) loc
...
betaR (TE tau (Op opr es loc))       idx n arg ts = TE (substitute ts tau) <$> (Op opr <$> mapM (\x -> betaR x idx n arg ts) es <*> pure loc)
...
betaR e@(TE tau (LetBang vs a e1 e2 loc)) idx n@(SSuc n0) arg ts
...
betaR (TE tau (If e1 e2 e3 loc)) idx n arg ts = TE (substitute ts tau) <$> (If <$> betaR e1 idx n arg ts <*> betaR e2 idx n arg ts <*> betaR e3 idx n arg ts <*> pure loc)
betaR (TE tau (Case e tn (l1,a1,e1) (l2,a2,e2) loc)) idx n arg ts = do
...
betaR (TE tau (Cast ty e loc))        idx n arg ts = TE (substitute ts tau) <$> (Cast (substitute ts ty) <$> betaR e idx n arg ts <*> pure loc)
#if __GLASGOW_HASKELL__ < 711
