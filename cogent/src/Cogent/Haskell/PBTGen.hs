...
mkAbsFBody cogIcTyp icTyp iaTyp 
    = let icLayout = determineUnpack cogIcTyp icTyp 0 "None"
          lens = mkLensView icLayout "ic" Nothing 
...
mkRrelBody cogOcTyp ocTyp oaTyp
    = let ocLy = determineUnpack cogOcTyp ocTyp 0 "None"
          ...
          ocLens = mkLensView ocLy "oc" Nothing 
          oaLens = mkLensView oaLy "oa" Nothing 
          ls = oaLens ++ ocLens 
          cNames = (getConNames ocLy []) ++ (getConNames oaLy [])
          binds = P.zip (map (\x -> pvar . mkName . fst $ x) ls) (map snd ls)
          body = function "undefined" -- packAbsCon iaTyp (map fst ls) 0
...
-- val = 
-- ----------------------------------------------------------------------------------------------------
mkLensView :: HsEmbedLayout -> String -> Maybe (Exp ()) -> [(String, (Exp ()))]
mkLensView tyL varToView prev
    = let hsTy = tyL ^. hsTyp
          group = tyL ^. grTag
          fld = tyL ^. fieldMap
          -- field map is a tree with int leaves -> only build vars for leaves
       in concat $ map ( \(k, v) -> case group of 
           HsPrim -> case v of
           ...
           _ -> case v of
              ...
              (Right next) -> mkLensView next varToView $ Just $ mkViewInfixE varToView group prev k
       ) (M.toList fld) 
...
determineUnpack' hsTyp depth fieldName | (TyApp _ l r) <- hsTyp 
    = let (maybeConName:fieldTypes) = unfoldAppCon hsTyp
          conName = case maybeConName of 
                      (TyCon _ (UnQual _ (Ident _ n))) -> n
                      _ -> __impossible $ "Bad Constructor name"++show l++"--"++show r
          groupTag = if | elem conName hsSumTypes -> HsVariant
                        | otherwise -> HsRecord
...
getAccessor conName groupTag ts fieldNames
    = let fs = case fieldNames of 
        ...
        in case groupTag of 
            HsRecord -> fs
            HsVariant -> if | conName == "Maybe" && P.length fs == 1 -> map (\x -> "_Just") fs 
...
isFromIntegral (TyCon _ (UnQual _ (Ident _ n))) 
    = elem n ["Word8","Word16","Word32","Word64"]

checkTy :: String -> [String] -> Bool
checkTy n xs = elem n xs
