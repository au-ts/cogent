...
import qualified Data.Set as S (delete, elems, toList)
import Lens.Micro
...
assignOf (t1@(T (TRefine v1 b1 p1)) :=: t2@(T (TRefine v2 b2 p2)))
  ...
  | HApp x v vs <- p1, null (unknownsE p2), all (`elem` vs) (S.toList $ progVars t2)
  = __assert_ (v == v1) ("assignOf: ill-formed ref.type: " ++ show (pretty t1)) $
      pure [ Subst.ofExpr x (substVarExpr [(v2,v)] p2) ]
  | HApp x v vs <- p2, null (unknownsE p1), all (`elem` vs) (S.toList $ progVars t1)
