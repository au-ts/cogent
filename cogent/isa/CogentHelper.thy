 ...
 *
 * The thms list is a list of kinding theorems which match the applications of kinding/weakening
 * needed to solve the weakening tac. Note that this list is de-duplicated.
 *)
fun weakening_tac ctxt (th :: thms) =
   REPEAT_DETERM 
      ((rtac @{thm weakening_cons} THEN' (
        (rtac @{thm weakening_comp.none})
        ORELSE'
        (rtac @{thm weakening_comp.drop} THEN' rtac th THEN' SOLVED' (simp_tac ctxt))
        ORELSE'
