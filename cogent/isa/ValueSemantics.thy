...
datatype ('f, 'g, 'a, 'b) vval = VPrim lit
                       | VProduct "('f, 'g, 'a, 'b) vval" "('f, 'g, 'a, 'b) vval"
                       | VSum name "('f, 'g, 'a, 'b) vval"
                       | VRecord "('f, 'g, 'a, 'b) vval list"
                       | VAbstract "'a"
                       | VDAbstract "'b"
                       | VFunction "('f, 'g) expr" "type list"
                       | VAFunction "'f" "type list"
                       | VDAFunction "'g" "type list"
                       | VUnit
...
type_synonym ('f, 'g, 'a, 'b) vabsfuns = "'f \<Rightarrow> ('f, 'g, 'a, 'b) vval \<Rightarrow> ('f, 'g, 'a, 'b) vval \<Rightarrow> bool"
type_synonym ('f, 'g, 'a, 'b) vdabsfuns = "'g \<Rightarrow> ('f, 'g, 'a, 'b) vval \<Rightarrow> ('f, 'g, 'a, 'b) vval \<Rightarrow> bool"

definition eval_prim :: "prim_op \<Rightarrow> ('f, 'g, 'a, 'b) vval list \<Rightarrow> ('f, 'g, 'a, 'b) vval"
where
...
assumes "(eval_prim :: prim_op \<Rightarrow> ('f1, 'g1, 'a1, 'b1) vval list \<Rightarrow> ('f1, 'g1, 'a1, 'b1) vval) p (map VPrim lits) = VPrim l"
shows "(eval_prim :: prim_op \<Rightarrow> ('f2, 'g2, 'a2, 'b2) vval list \<Rightarrow> ('f2, 'g2, 'a2, 'b2) vval) p (map VPrim  lits) = VPrim l"
proof -
...
inductive v_sem :: "('f, 'g, 'a, 'b) vdabsfuns \<Rightarrow> ('f, 'g, 'a, 'b) vabsfuns \<Rightarrow> ('f, 'g, 'a, 'b) vval env \<Rightarrow> ('f, 'g) expr \<Rightarrow> ('f, 'g, 'a, 'b) vval \<Rightarrow> bool"
          ("_, _ , _ \<turnstile> _ \<Down> _" [30, 30,0,0,20] 60)
and       v_sem_all  :: "('f, 'g, 'a, 'b) vdabsfuns \<Rightarrow> ('f, 'g, 'a, 'b) vabsfuns \<Rightarrow> ('f, 'g, 'a, 'b) vval list \<Rightarrow> ('f, 'g) expr list \<Rightarrow> ('f, 'g, 'a, 'b) vval list \<Rightarrow> bool"
          ("_, _ , _ \<turnstile>* _ \<Down> _" [30, 30,0,0,20] 60)
where
  v_sem_var     : "\<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> (Var i) \<Down> (\<gamma> ! i)"

| v_sem_lit     : "\<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> (Lit l) \<Down> VPrim l"

| v_sem_prim    : "\<lbrakk> \<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile>* as \<Down> as'
                   \<rbrakk> \<Longrightarrow>  \<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> (Prim p as) \<Down> eval_prim p as'"

| v_sem_fun     : "\<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> Fun f ts \<Down> VFunction f ts"

| v_sem_afun     : "\<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> AFun f ts \<Down> VAFunction f ts"

| v_sem_dafun     : "\<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> DAFun f ts \<Down> VDAFunction f ts"

| v_sem_abs_app : "\<lbrakk> \<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> x \<Down> VAFunction f ts
                   ; \<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> y \<Down> a
                   ; \<xi> f a r
                   \<rbrakk> \<Longrightarrow> \<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> (App x y) \<Down> r"

| v_sem_dabs_app : "\<lbrakk> \<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> x \<Down> VDAFunction g ts
                   ; \<xi>\<^sub>d , \<xi>, \<gamma> \<turnstile> y \<Down> a
                   ; \<xi>\<^sub>d g a r
                   \<rbrakk> \<Longrightarrow> \<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> (App x y) \<Down> r"

| v_sem_cast    : "\<lbrakk> \<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> e \<Down> VPrim l
                   ; cast_to \<tau> l = Some l'
                   \<rbrakk> \<Longrightarrow> \<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> Cast \<tau> e \<Down> VPrim l'"

| v_sem_app     : "\<lbrakk> \<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> x \<Down> VFunction e ts
                   ; \<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> y \<Down> a
                   ; \<xi>\<^sub>d , \<xi> , [ a ] \<turnstile> specialise ts e \<Down> r
                   \<rbrakk> \<Longrightarrow> \<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> (App x y) \<Down> r"

| v_sem_con     : "\<lbrakk> \<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> x \<Down> x'
                   \<rbrakk> \<Longrightarrow> \<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> (Con _ t x) \<Down> VSum t x'"

| v_sem_member  : "\<lbrakk> \<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> e \<Down> VRecord fs
                   \<rbrakk> \<Longrightarrow> \<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> Member e f \<Down> fs ! f"

| v_sem_unit    : "\<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> Unit \<Down> VUnit"

| v_sem_tuple   : "\<lbrakk> \<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> x \<Down> x'
                   ; \<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> y \<Down> y'
                   \<rbrakk> \<Longrightarrow> \<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> (Tuple x y) \<Down> VProduct x' y'"

| v_sem_esac    : "\<lbrakk> \<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> t \<Down> VSum ts v
                   \<rbrakk> \<Longrightarrow> \<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> Esac t ts \<Down> v"

| v_sem_let     : "\<lbrakk> \<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> a \<Down> a'
                   ; \<xi>\<^sub>d , \<xi> , (a' # \<gamma>) \<turnstile> b \<Down> b'
                   \<rbrakk> \<Longrightarrow> \<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> Let a b \<Down> b'"

| v_sem_letbang : "\<lbrakk> \<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> a \<Down> a'
                   ; \<xi>\<^sub>d , \<xi> , (a' # \<gamma>) \<turnstile> b \<Down> b'
                   \<rbrakk> \<Longrightarrow> \<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> LetBang vs a b \<Down> b'"

| v_sem_case_m  : "\<lbrakk> \<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> x \<Down> VSum t v
                   ; \<xi>\<^sub>d , \<xi> , (v # \<gamma>) \<turnstile> m \<Down> m'
                   \<rbrakk> \<Longrightarrow> \<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> Case x t m n \<Down> m'"

| v_sem_case_nm : "\<lbrakk> \<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> x \<Down> VSum t' v
                   ; t \<noteq> t'
                   ; \<xi>\<^sub>d , \<xi> , (VSum t' v # \<gamma>) \<turnstile> n \<Down> n'
                   \<rbrakk> \<Longrightarrow> \<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> Case x t m n \<Down> n'"

| v_sem_if      : "\<lbrakk> \<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> x \<Down> VPrim (LBool b)
                   ; \<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> if b then t else e \<Down> r
                   \<rbrakk> \<Longrightarrow> \<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> If x t e \<Down> r"

| v_sem_struct  : "\<lbrakk> \<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile>* xs \<Down> vs
                   \<rbrakk> \<Longrightarrow> \<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> Struct ts xs \<Down> VRecord vs"

| v_sem_take    : "\<lbrakk> \<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> x \<Down> VRecord fs
                   ; \<xi>\<^sub>d , \<xi> , (fs ! f # VRecord fs # \<gamma>) \<turnstile> e \<Down> e'
                   \<rbrakk> \<Longrightarrow> \<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> Take x f e \<Down> e'"

| v_sem_put     : "\<lbrakk> \<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> x \<Down> VRecord fs
                   ; \<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> e \<Down> e'
                   \<rbrakk> \<Longrightarrow> \<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> Put x f e \<Down> VRecord (fs [ f := e' ])"

| v_sem_split   : "\<lbrakk> \<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> x \<Down> VProduct a b
                   ; \<xi>\<^sub>d , \<xi> , (a # b # \<gamma>) \<turnstile> e \<Down> e'
                   \<rbrakk> \<Longrightarrow> \<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> Split x e \<Down> e'"

| v_sem_promote : "\<lbrakk> \<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> e \<Down> e'
                   \<rbrakk> \<Longrightarrow> \<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> Promote t' e \<Down> e'"


| v_sem_all_empty : "\<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile>* [] \<Down> []"

| v_sem_all_cons  : "\<lbrakk> \<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> x \<Down> v
                     ; \<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile>* xs \<Down> vs
                     \<rbrakk> \<Longrightarrow> \<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile>* (x # xs) \<Down> (v # vs)"

inductive_cases v_sem_varE  [elim] : "\<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> Var i \<Down> v"
inductive_cases v_sem_funE  [elim] : "\<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> Fun f ts \<Down> v"
inductive_cases v_sem_afunE [elim] : "\<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> AFun f ts \<Down> v"
inductive_cases v_sem_appE  [elim] : "\<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> App a b \<Down> v"
...
inductive vval_typing  :: "('g \<Rightarrow> poly_type) \<Rightarrow> ('f \<Rightarrow> poly_type) \<Rightarrow> ('f, 'g, 'a, 'b) vval \<Rightarrow> type \<Rightarrow> bool"
          ("_, _ \<turnstile> _ :v _" [30,0,20] 80)
and vval_typing_record :: "('g \<Rightarrow> poly_type) \<Rightarrow> ('f \<Rightarrow> poly_type) \<Rightarrow> ('f, 'g, 'a, 'b) vval list \<Rightarrow> (name \<times> type \<times> record_state) list \<Rightarrow> bool"
          ("_, _ \<turnstile>* _ :vr _" [30,0,20] 80) where

  v_t_prim     : " \<Xi>\<^sub>d, \<Xi> \<turnstile> VPrim l :v TPrim (lit_type l)"

| v_t_product  : "\<lbrakk>  \<Xi>\<^sub>d, \<Xi> \<turnstile> a :v t
                  ;  \<Xi>\<^sub>d, \<Xi> \<turnstile> b :v u
                  \<rbrakk> \<Longrightarrow>  \<Xi>\<^sub>d, \<Xi> \<turnstile> VProduct a b :v TProduct t u"

| v_t_sum      : "\<lbrakk>  \<Xi>\<^sub>d, \<Xi> \<turnstile> a :v t
                  ; (g, t, Unchecked) \<in> set ts
                  ; distinct (map fst ts)
                  ; [] \<turnstile> TSum ts wellformed
                  \<rbrakk> \<Longrightarrow>  \<Xi>\<^sub>d, \<Xi> \<turnstile> VSum g a :v TSum ts"

| v_t_record   : "\<lbrakk>  \<Xi>\<^sub>d, \<Xi> \<turnstile>* fs :vr ts
                  ; distinct (map fst ts)
                  \<rbrakk> \<Longrightarrow>  \<Xi>\<^sub>d, \<Xi> \<turnstile> VRecord fs :v TRecord ts s"

| v_t_abstract : "\<lbrakk> abs_typing a n ts
                  ; [] \<turnstile>* ts wellformed
                  \<rbrakk> \<Longrightarrow>  \<Xi>\<^sub>d, \<Xi> \<turnstile> VAbstract a :v TCon n ts s"

| v_t_dabstract : "\<lbrakk> abs_dtyping b n1 n2 n3 ts
                  ; [] \<turnstile>* ts wellformed
                  \<rbrakk> \<Longrightarrow>  \<Xi>\<^sub>d, \<Xi> \<turnstile> VDAbstract b :v TDCon n1 n2 n3 ts s"
...
(*
  ...
  | v_t_subsumption: " \<lbrakk>  \<Xi>\<^sub>d, \<Xi> \<turnstile> v :v t ; [] \<turnstile> t \<sqsubseteq> t' \<rbrakk> \<Longrightarrow>  \<Xi>\<^sub>d, \<Xi> \<turnstile> v :v t' "
...
| v_t_afun     : "\<lbrakk> \<Xi> f = (ks, a, b)
                  ...
                  \<rbrakk> \<Longrightarrow>  \<Xi>\<^sub>d, \<Xi> \<turnstile> VAFunction f ts :v TFun t' u'"

| v_t_dafun     : "\<lbrakk> \<Xi>\<^sub>d f = (ks, a, b)
                  ; list_all2 (kinding []) ts ks
                  ; ks \<turnstile> TDFun a b wellformed
                  ; [] \<turnstile> TDFun (instantiate ts a) (instantiate ts b) \<sqsubseteq> TDFun t' u'
                  \<rbrakk> \<Longrightarrow>  \<Xi>\<^sub>d, \<Xi> \<turnstile> VDAFunction f ts :v TDFun t' u'"

| v_t_function : "\<lbrakk> \<Xi>\<^sub>d, \<Xi> , K , [ Some t ] \<turnstile> f : u
                  ; K \<turnstile> t wellformed
                  ; list_all2 (kinding []) ts K
                  ; [] \<turnstile> TFun (instantiate ts t) (instantiate ts u) \<sqsubseteq> TFun t' u'
                  \<rbrakk> \<Longrightarrow>  \<Xi>\<^sub>d, \<Xi> \<turnstile> VFunction f ts :v TFun t' u'"

| v_t_unit     : " \<Xi>\<^sub>d, \<Xi> \<turnstile> VUnit :v TUnit"

| v_t_r_empty  : " \<Xi>\<^sub>d, \<Xi> \<turnstile>* [] :vr []"
| v_t_r_cons1  : "\<lbrakk>  \<Xi>\<^sub>d, \<Xi> \<turnstile> x :v t
                  ;  \<Xi>\<^sub>d, \<Xi> \<turnstile>* xs :vr ts
                  \<rbrakk> \<Longrightarrow>  \<Xi>\<^sub>d, \<Xi> \<turnstile>* (x # xs) :vr ((n, t, Present) # ts)"
| v_t_r_cons2  : "\<lbrakk> [] \<turnstile> t wellformed
                  ;  \<Xi>\<^sub>d, \<Xi> \<turnstile>* xs :vr ts
                  \<rbrakk> \<Longrightarrow>  \<Xi>\<^sub>d, \<Xi> \<turnstile>* (x # xs) :vr ((n, t, Taken) # ts)"


lemma v_t_prim' [intro]:
assumes "lit_type l = \<tau>"
shows   " \<Xi>\<^sub>d, \<Xi> \<turnstile> VPrim l :v TPrim \<tau>"
using assms by (auto intro: v_t_prim)

inductive_cases v_t_funE      [elim]: " \<Xi>\<^sub>d, \<Xi> \<turnstile> VFunction f ts :v t"
inductive_cases v_t_afunE     [elim]: " \<Xi>\<^sub>d, \<Xi> \<turnstile> VAFunction f ts :v t"
inductive_cases v_t_recordE   [elim]: " \<Xi>\<^sub>d, \<Xi> \<turnstile> VRecord fs :v \<tau>"
inductive_cases v_t_productE  [elim]: " \<Xi>\<^sub>d, \<Xi> \<turnstile> VProduct a b :v \<tau>"
inductive_cases v_t_sumE'     [elim]: " \<Xi>\<^sub>d, \<Xi> \<turnstile> e :v TSum ts"
inductive_cases v_t_primE     [elim]: " \<Xi>\<^sub>d, \<Xi> \<turnstile> VPrim l :v TPrim (Num \<tau>)"

inductive_cases v_t_r_emptyE  [elim]: " \<Xi>\<^sub>d, \<Xi> \<turnstile>* [] :vr \<tau>s"
inductive_cases v_t_r_consE   [elim]: " \<Xi>\<^sub>d, \<Xi> \<turnstile>* (x # xs) :vr \<tau>s"


definition 
  vval_typing_all :: "('g \<Rightarrow> poly_type) \<Rightarrow> ('f \<Rightarrow> poly_type) \<Rightarrow> ('f, 'g, 'a, 'b) vval list \<Rightarrow> type list \<Rightarrow> bool"
           ("_, _  \<turnstile>* _ :v _" [30,0,20] 80) where
   "(\<Xi>\<^sub>d, \<Xi> \<turnstile>* vs :v ts) \<equiv> list_all2 (vval_typing  \<Xi>\<^sub>d \<Xi>) vs ts"

definition matches :: "('g \<Rightarrow> poly_type) \<Rightarrow> ('f \<Rightarrow> poly_type) \<Rightarrow>  ('f, 'g, 'a, 'b) vval env \<Rightarrow> ctx \<Rightarrow> bool"
           ("_, _ \<turnstile> _ matches _" [30,0,20] 60) where
   "\<Xi>\<^sub>d, \<Xi> \<turnstile> \<gamma> matches \<Gamma> \<equiv> list_all2 (\<lambda> x m. \<forall> \<tau>. m = Some \<tau> \<longrightarrow> \<Xi>\<^sub>d, \<Xi> \<turnstile> x :v \<tau>) \<gamma> \<Gamma>"

lemmas matches_Cons = 
  list_all2_Cons[where P="(\<lambda>x m. \<forall>\<tau>. m = Some \<tau> \<longrightarrow> \<Xi>\<^sub>d, \<Xi> \<turnstile> x :v \<tau>)" for \<Xi> \<Xi>\<^sub>d, 
                 simplified matches_def[symmetric]]

definition proc_env_matches :: "('f \<Rightarrow> ('f, 'g, 'a, 'b) vval \<Rightarrow> ('f, 'g, 'a, 'b) vval \<Rightarrow> bool) \<Rightarrow> ('f \<Rightarrow> poly_type) \<Rightarrow> bool"
           ("_ matches _" [30,20] 60) where
  "\<xi> matches \<Xi> \<equiv> (\<forall> f. let (K, \<tau>i, \<tau>o) = \<Xi> f
                        in (\<forall> \<Xi>\<^sub>d \<tau>s v v'. list_all2 (kinding []) \<tau>s K
                                  \<longrightarrow> (\<Xi>\<^sub>d , \<Xi> \<turnstile> v  :v instantiate \<tau>s \<tau>i)
                                  \<longrightarrow> \<xi> f v v'
                                  \<longrightarrow> (\<Xi>\<^sub>d , \<Xi> \<turnstile> v' :v instantiate \<tau>s \<tau>o)))"


section {* vval_typing lemmas *}

lemma vval_typing_to_wellformed:
  shows "\<Xi>\<^sub>d , \<Xi> \<turnstile> v :v \<tau>     \<Longrightarrow> [] \<turnstile> \<tau> wellformed"
    and "\<Xi>\<^sub>d , \<Xi> \<turnstile>* vs :vr fs \<Longrightarrow> [] \<turnstile>* map (fst \<circ> snd) fs wellformed"
...
next case v_t_dafun  then show ?case
    by (fastforce dest: subtyping_wellformed_preservation(1) list_all2_kinding_wellformedD dest:instantiate_wellformed)
...
lemma vval_typing_bang:
  shows "\<Xi>\<^sub>d , \<Xi> \<turnstile> x :v \<tau> \<Longrightarrow> \<Xi>\<^sub>d , \<Xi> \<turnstile> x :v bang \<tau>"
    and "\<Xi>\<^sub>d , \<Xi> \<turnstile>* fs :vr \<tau>rs \<Longrightarrow> \<Xi>\<^sub>d , \<Xi> \<turnstile>* fs :vr map (\<lambda>(n, x, y). (n, bang x, y)) \<tau>rs"
proof (induct rule: vval_typing_vval_typing_record.inducts)
...
assumes "\<Xi>\<^sub>d , \<Xi> \<turnstile>* fs :vr \<tau>s"
...
and     "f < length \<tau>s"
shows   "\<Xi>\<^sub>d , \<Xi> \<turnstile> fs ! f :v \<tau>"
...
lemma vval_typing_all_record:
  assumes "\<Xi>\<^sub>d , \<Xi> \<turnstile>* vs :v ts"
  and "length ns = length ts"
shows "\<Xi>\<^sub>d , \<Xi> \<turnstile>* vs :vr zip ns (zip ts (replicate (length ts) Present))"
...
lemma vval_typing_record_take:
  assumes "\<Xi>\<^sub>d , \<Xi> \<turnstile>* ts :vr \<tau>s"
    and "\<tau>s ! f = (n, t, Present)"
    and "[] \<turnstile> t :\<kappa> k"
    and "S \<in> k \<or> taken = Taken"
  shows "\<Xi>\<^sub>d , \<Xi> \<turnstile>* ts :vr \<tau>s[ f := (n, t, taken) ]"
  using assms
...
and     "\<Xi>\<^sub>d , \<Xi> \<turnstile> v :v t"
shows   "\<Xi>\<^sub>d , \<Xi> \<turnstile>* ts[ f := v ] :vr \<tau>s[ f := (n, t, Present) ]"
...
and     "\<Xi>\<^sub>d , \<Xi> \<turnstile> v :v TSum ts"
and     "[] \<turnstile> TSum us wellformed"
shows   "\<Xi>\<^sub>d , \<Xi> \<turnstile> v :v TSum us"
using assms
by (force simp add: kinding_simps intro: vval_typing_vval_typing_variant_vval_typing_record.intros)
*)
lemma sum_downcast:
  assumes vval_tsum_ts: "\<Xi>\<^sub>d , \<Xi> \<turnstile> VSum tag v :v TSum ts"
    and   tag_neq_tag': "tag \<noteq> tag'"
    and   tag'_in_ts  : "(tag', \<tau>, Unchecked) \<in> set ts"
  shows "\<Xi>\<^sub>d , \<Xi> \<turnstile> VSum tag v :v TSum (tagged_list_update tag' (\<tau>, Checked) ts)"
proof -
  from vval_tsum_ts
  obtain \<tau>1
    where vval_elim_lemmas:
      "\<Xi>\<^sub>d , \<Xi> \<turnstile> v :v \<tau>1"
...
lemma value_subtyping_to_wellformed:
  ...
  \<Longrightarrow> \<Xi>\<^sub>d , \<Xi> \<turnstile> v :v t
...
lemma value_subtyping:
  shows "\<Xi>\<^sub>d , \<Xi> \<turnstile> v :v t \<Longrightarrow>[] \<turnstile> t \<sqsubseteq> t' \<Longrightarrow> \<Xi>\<^sub>d , \<Xi> \<turnstile> v :v t'"
    and "\<Xi>\<^sub>d , \<Xi> \<turnstile>* vs :vr ts \<Longrightarrow> [] \<turnstile> TRecord ts s \<sqsubseteq> TRecord ts' s \<Longrightarrow>  \<Xi>\<^sub>d , \<Xi> \<turnstile>* vs :vr ts'"
proof (induct arbitrary: t' and ts' rule: vval_typing_vval_typing_record.inducts)
  case (v_t_product \<Xi>\<^sub>d \<Xi> va ta vb tb)
...
next
  case (v_t_sum \<Xi>\<^sub>d  \<Xi> a ta n ts1)
...
next
  case (v_t_afun \<Xi>\<^sub>d \<Xi> f ks ta tb ts tfun')
...
next
  case (v_t_dafun \<Xi>\<^sub>d \<Xi> f ks ta tb ts tfun')
  then obtain tx ux where "t' = TDFun tx ux"
    by (auto elim: subtyping.cases)
  then show ?case
    using v_t_dafun by (meson subtyping_trans vval_typing_vval_typing_record.intros subtyping_simps)
next
...
next
  ...
  then show ?case
    by (auto elim: subtyping.cases intro: vval_typing_vval_typing_record.intros)
next
  case (v_t_r_cons1 \<Xi>\<^sub>d \<Xi> x t1 xs ts n)
  ...
  have field_rest: "\<Xi>\<^sub>d , \<Xi> \<turnstile>* xs :vr ts2'"
  ...
  qed
next
  case (v_t_r_cons2 t1 \<Xi>\<^sub>d \<Xi> xs ts x n)
...
and     "\<Xi> f = (K, t, u)"
shows   "\<Xi>\<^sub>d , \<Xi> \<turnstile> VAFunction f (map (instantiate \<delta>) ts) :v TFun (instantiate \<delta> (instantiate ts t))
...
lemma v_t_dafun_instantiate:
assumes "list_all2 (kinding K') ts K"
and     "list_all2 (kinding []) \<delta> K'"
and     "K \<turnstile> t wellformed"
and     "K \<turnstile> u wellformed"
and     "\<Xi>\<^sub>d g = (K, t, u)"
shows   "\<Xi>\<^sub>d , \<Xi> \<turnstile> VDAFunction g (map (instantiate \<delta>) ts) :v TDFun (instantiate \<delta> (instantiate ts t))
                                                           (instantiate \<delta> (instantiate ts u))"
proof -
  from assms
  have tdfun_eq:
      "TDFun (instantiate \<delta> (instantiate ts t))
             (instantiate \<delta> (instantiate ts u))
      = TDFun (instantiate (map (instantiate \<delta>) ts) t)
             (instantiate (map (instantiate \<delta>) ts) u)"
    by (force intro: instantiate_instantiate dest: list_all2_lengthD)

  have tdfun_sub:
    "[] \<turnstile> TDFun (instantiate (map (instantiate \<delta>) ts) t) (instantiate (map (instantiate \<delta>) ts) u)
        \<sqsubseteq> TDFun (instantiate       \<delta> (instantiate ts t)) (instantiate       \<delta> (instantiate ts u))"
    using assms tdfun_eq
    by (metis subtyping_refl)

  show ?thesis
    using assms tdfun_sub 
    by (meson list_all2_substitutivity type_wellformed_pretty_intros(6) v_t_dafun)
qed

lemma v_t_function_instantiate:
  assumes "\<Xi>\<^sub>d , \<Xi>, K, [Some t] \<turnstile> f : u"
  and     "K \<turnstile> t wellformed"
  and     "list_all2 (kinding []) \<delta> K'"
  assumes "list_all2 (kinding K') ts K"
  shows   "\<Xi>\<^sub>d , \<Xi> \<turnstile> VFunction f (map (instantiate \<delta>) ts) :v TFun (instantiate \<delta> (instantiate ts t))
                                                            (instantiate \<delta> (instantiate ts u))"
proof -
...
and     "\<Xi>\<^sub>d, \<Xi> \<turnstile> \<gamma> matches \<Gamma>"
shows   "\<Xi>\<^sub>d, \<Xi> \<turnstile> \<gamma> matches \<Gamma>1"
and     "\<Xi>\<^sub>d, \<Xi> \<turnstile> \<gamma> matches \<Gamma>2"
proof -
  have "\<And>a x y z. [] \<turnstile> x \<leadsto> y \<parallel> z \<Longrightarrow> \<forall>\<tau>. x = Some \<tau> \<longrightarrow> \<Xi>\<^sub>d, \<Xi> \<turnstile> a :v \<tau> \<Longrightarrow> 
              (\<forall>\<tau>. y = Some \<tau> \<longrightarrow> \<Xi>\<^sub>d, \<Xi> \<turnstile> a :v \<tau>) \<and> (\<forall>\<tau>. z = Some \<tau> \<longrightarrow> \<Xi>\<^sub>d, \<Xi> \<turnstile> a :v \<tau>)"
    by (force simp add: split_comp.simps)
  then show "\<Xi>\<^sub>d, \<Xi> \<turnstile> \<gamma> matches \<Gamma>1" "\<Xi>\<^sub>d, \<Xi> \<turnstile> \<gamma> matches \<Gamma>2"
    using list_all3_product_over_list_all2
      [where A="\<lambda>x m. \<forall>\<tau>. m = Some \<tau> \<longrightarrow> \<Xi>\<^sub>d, \<Xi> \<turnstile> x :v \<tau>" and P="split_comp []",
...
lemma matches_split:
assumes "\<Xi>\<^sub>d, \<Xi> \<turnstile> \<gamma> matches (instantiate_ctx \<tau>s \<Gamma>)"
and     "list_all2 (kinding []) \<tau>s K"
and     "K \<turnstile> \<Gamma> \<leadsto> \<Gamma>1 | \<Gamma>2"
shows   "\<Xi>\<^sub>d, \<Xi> \<turnstile> \<gamma> matches (instantiate_ctx \<tau>s \<Gamma>1)"
and     "\<Xi>\<^sub>d, \<Xi> \<turnstile> \<gamma> matches (instantiate_ctx \<tau>s \<Gamma>2)"
...
and     "list_all2 (kinding []) \<tau>s K"
and     "K \<turnstile> \<Gamma> \<leadsto> \<Gamma>1 | \<Gamma>2"
shows   "(\<Xi>\<^sub>d, \<Xi> \<turnstile> \<gamma> matches (instantiate_ctx \<tau>s \<Gamma>1)) \<and> (\<Xi>\<^sub>d, \<Xi> \<turnstile> \<gamma> matches (instantiate_ctx \<tau>s \<Gamma>2))"
...
and     "list_all2 (kinding []) \<tau>s K"
and     "K \<turnstile> \<Gamma> \<leadsto> \<Gamma>1 | \<Gamma>2"
and     "\<lbrakk> \<Xi>\<^sub>d, \<Xi> \<turnstile> \<gamma> matches (instantiate_ctx \<tau>s \<Gamma>1) ; \<Xi>\<^sub>d, \<Xi> \<turnstile> \<gamma> matches (instantiate_ctx \<tau>s \<Gamma>2) \<rbrakk> \<Longrightarrow> P"
shows   "P"
...
assumes "\<Xi>\<^sub>d, \<Xi> \<turnstile> \<gamma> matches \<Gamma>"
and     "[] \<turnstile> \<Gamma> \<leadsto>w \<Gamma>'"
shows   "\<Xi>\<^sub>d, \<Xi> \<turnstile> \<gamma> matches \<Gamma>'"
...
shows   "\<Xi>\<^sub>d, \<Xi> \<turnstile> \<gamma> matches (instantiate_ctx \<tau>s \<Gamma>')"
using assms by (auto dest: instantiate_ctx_weaken intro: matches_weakening')

lemma matches_cons':
assumes "\<Xi>\<^sub>d, \<Xi> \<turnstile> \<gamma> matches \<Gamma>"
and     "\<Xi>\<^sub>d, \<Xi> \<turnstile> x :v \<tau>"
shows   "\<Xi>\<^sub>d, \<Xi> \<turnstile> (x # \<gamma>) matches (Some \<tau> # \<Gamma>)"
using assms by (simp add: matches_def)

lemma matches_cons:
assumes "list_all2 (kinding []) \<tau>s K"
and     "\<Xi>\<^sub>d, \<Xi> \<turnstile> \<gamma> matches (instantiate_ctx \<tau>s \<Gamma>)"
and     "\<Xi>\<^sub>d, \<Xi> \<turnstile> x :v instantiate \<tau>s \<tau>"
shows   "\<Xi>\<^sub>d, \<Xi> \<turnstile> (x # \<gamma>) matches (instantiate_ctx \<tau>s (Some \<tau> # \<Gamma>))"
using assms by (auto intro: matches_cons')

lemma matches_empty':
shows "\<Xi>\<^sub>d, \<Xi> \<turnstile> [] matches []"
by (simp add: matches_def)

lemma matches_empty:
shows "\<Xi>\<^sub>d, \<Xi> \<turnstile> [] matches instantiate_ctx \<tau>s []"
...
shows   "\<Xi>\<^sub>d, \<Xi> \<turnstile> (\<gamma> ! i) :v \<tau>"
...
and     "i < length \<Gamma>"
and     "\<Gamma> ! i = Some \<tau>"
shows   "\<Xi>\<^sub>d, \<Xi> \<turnstile> (\<gamma> ! i) :v instantiate \<tau>s \<tau>"
...
and     "list_all2 (kinding []) \<tau>s K"
and     "\<Xi>\<^sub>d, \<Xi> \<turnstile> v    :v instantiate \<tau>s \<tau>i"
and     "\<xi> f v v'"
shows   "\<Xi>\<^sub>d, \<Xi> \<turnstile> v' :v instantiate \<tau>s \<tau>o"
...
theorem progress:
assumes "\<Xi>\<^sub>d, \<Xi>, K, \<Gamma> \<turnstile> e : \<tau>"
and     "\<xi> matches \<Xi>"
and     "list_all2 (kinding []) \<tau>s K"
and     "\<Xi>\<^sub>d, \<Xi> \<turnstile> \<gamma> matches (instantiate_ctx \<tau>s \<Gamma>)"
shows   "\<exists>! v. \<xi>\<^sub>d, \<xi>, \<gamma> \<turnstile> specialise \<tau>s e \<Down> v"
oops

lemma v_t_map_TPrimD:
  "\<Xi>\<^sub>d, \<Xi> \<turnstile>* vs :v map TPrim \<tau>s
...
and     "\<Xi>\<^sub>d, \<Xi> \<turnstile>* vs :v map TPrim \<tau>s"
shows   "\<Xi>\<^sub>d, \<Xi> \<turnstile>  eval_prim p vs :v TPrim \<tau>"
...
and     "\<Xi>\<^sub>d, \<Xi> \<turnstile> \<gamma> matches (instantiate_ctx \<tau>s \<Gamma>)"
and     "\<xi> matches \<Xi>"
shows   "\<lbrakk> \<xi>\<^sub>d, \<xi>, \<gamma> \<turnstile>  specialise \<tau>s e \<Down> v  ; \<Xi>\<^sub>d, \<Xi>, K, \<Gamma> \<turnstile>  e  : \<tau> \<rbrakk> \<Longrightarrow>  \<Xi>\<^sub>d, \<Xi> \<turnstile>  v  :v instantiate \<tau>s \<tau>"
and     "\<lbrakk> \<xi>\<^sub>d, \<xi>, \<gamma> \<turnstile>* map (specialise \<tau>s) es \<Down> vs ; \<Xi>\<^sub>d, \<Xi>, K, \<Gamma> \<turnstile>* es : \<tau>s' \<rbrakk> \<Longrightarrow> \<Xi>\<^sub>d, \<Xi> \<turnstile>* vs :v map (instantiate \<tau>s) \<tau>s'"
...
next case v_sem_dafun    then show ?case by ( case_tac e, simp_all
                                           , fastforce intro: v_t_dafun_instantiate simp add: kinding_simps)
next case v_sem_fun     then show ?case by ( case_tac e, simp_all
                                           , fastforce intro: v_t_function_instantiate)
next case (v_sem_con \<xi>\<^sub>d \<xi> \<gamma> x_spec x' ts_inst tag)
  then show ?case
  proof (cases e)
    case (Con ts tag' x)
    ...
    moreover then obtain t
      where con_elims:
        "\<tau> = TSum ts"
        "\<Xi>\<^sub>d, \<Xi>, K, \<Gamma> \<turnstile> x : t"
        "distinct (map fst ts)"
        "K \<turnstile> TSum ts wellformed"
        "(tag, t, Unchecked) \<in> set ts"
      using Con v_sem_con.prems
      by auto
    ultimately have "\<Xi>\<^sub>d, \<Xi> \<turnstile> VSum tag x' :v TSum (map (\<lambda>(c, t, b). (c, instantiate \<tau>s t, b)) ts)"
    ...
    next
      show "[] \<turnstile> TSum (map (\<lambda>(c, t, b). (c, instantiate \<tau>s t, b)) ts) wellformed"
        ...
        by (metis instantiate.simps(8) kinding_iff_wellformed(1) substitutivity_single)
    qed auto
    then show ?thesis
...
next case v_sem_case_m  then show ?case by ( case_tac e, simp_all
                                           , fastforce intro: matches_split
                                                       ...
                                                       dest:  distinct_fst)
next case (v_sem_case_nm \<xi>\<^sub>d  \<xi> \<gamma> x tag' v tag n n' m)
  ...
  proof (case_tac e; clarsimp)
    ...
    then obtain \<Gamma>1 \<Gamma>2 ts ta
      ...
      and typing_x1: "\<Xi>\<^sub>d, \<Xi>, K, \<Gamma>1 \<turnstile> x1 : TSum ts"
      and ta_in_ts: "(tag, ta, Unchecked) \<in> set ts"
      and typing_x3: "\<Xi>\<^sub>d, \<Xi>, K, Some ta # \<Gamma>2 \<turnstile> x3 : \<tau>"
      and typing_x4: "\<Xi>\<^sub>d, \<Xi>, K, Some (TSum (tagged_list_update tag (ta, Checked) ts)) # \<Gamma>2 \<turnstile> x4 : \<tau>"
      using v_sem_case_nm.prems
      by fastforce

    from split\<Gamma>
    have \<gamma>_matches_\<Gamma>1: "\<Xi>\<^sub>d, \<Xi> \<turnstile> \<gamma> matches instantiate_ctx \<tau>s \<Gamma>1"
      and \<gamma>_matches_\<Gamma>2: "\<Xi>\<^sub>d, \<Xi> \<turnstile> \<gamma> matches instantiate_ctx \<tau>s \<Gamma>2"
      using matches_split2 v_sem_case_nm.prems
      by fastforce+

    have "\<Xi>\<^sub>d, \<Xi> \<turnstile> VSum tag' v :v instantiate \<tau>s (TSum ts)"
      using x_is typing_x1 \<gamma>_matches_\<Gamma>1 v_sem_case_nm.hyps(2) v_sem_case_nm.prems
      by fastforce
    then have "\<Xi>\<^sub>d, \<Xi> \<turnstile> VSum tag' v :v TSum (tagged_list_update tag (instantiate \<tau>s ta, Checked) (map (\<lambda>(c, t, b). (c, instantiate \<tau>s t, b)) ts))"
      using v_sem_case_nm.hyps(3) image_iff ta_in_ts
      by (fastforce intro!: sum_downcast)
    then have "\<Xi>\<^sub>d, \<Xi> \<turnstile> VSum tag' v :v instantiate \<tau>s (TSum (tagged_list_update tag (ta, Checked) ts))"
    ...
    then show "\<Xi>\<^sub>d, \<Xi> \<turnstile> n' :v instantiate \<tau>s \<tau>"
      using v_sem_case_nm.prems v_sem_case_nm.hyps(5) \<gamma>_matches_\<Gamma>2 matches_cons
        n_is typing_x4
      by blast
  qed
next
  case (v_sem_esac \<xi>\<^sub>d \<xi> \<gamma> spec_a tag v)
  then show ?case
  proof (cases e)
    case (Esac a)
    ...
    then obtain ts' tag'
      where esac_elims:
        "\<Xi>\<^sub>d, \<Xi>, K, \<Gamma> \<turnstile> a : TSum ts'"
    ...
    then obtain \<tau>'
      where ih_elims:
        "\<Xi>\<^sub>d, \<Xi> \<turnstile> v :v instantiate \<tau>s \<tau>'"
    ...
    then show "\<Xi>\<^sub>d, \<Xi> \<turnstile> v :v instantiate \<tau>s \<tau>"
...
next case v_sem_struct  then show ?case by ( case_tac e, simp_all
                                           , fastforce intro: vval_typing_vval_typing_record.intros
                                                              vval_typing_all_record [ where ts = "map f ts" for f ts
                                                                                     , simplified])
next
  case (v_sem_take \<xi>\<^sub>d \<xi> \<gamma> spec_x fs f' spec_y e')
  then show ?case
  proof (cases e)
    case (Take x f y)
    ...
    obtain \<Gamma>1 \<Gamma>2 ts s n t k taken
      where typing_elims:
        ...
        "\<Xi>\<^sub>d, \<Xi>, K, \<Gamma>1 \<turnstile> x : TRecord ts s"
        "sigil_perm s \<noteq> Some ReadOnly"
        "f < length ts"
        "ts ! f = (n, t, Present)"
        "K \<turnstile> t :\<kappa> k"
        "S \<in> k \<or> taken = Taken"
        "\<Xi>\<^sub>d, \<Xi>, K, Some t # Some (TRecord (ts[f := (n, t, taken)]) s) # \<Gamma>2 \<turnstile> y : \<tau>"
      using v_sem_take.prems(1) spec_simps Take
      by blast
    then have matchsplit_lemmas:
      "\<Xi>\<^sub>d, \<Xi> \<turnstile> \<gamma> matches instantiate_ctx \<tau>s \<Gamma>1"
      "\<Xi>\<^sub>d, \<Xi> \<turnstile> \<gamma> matches instantiate_ctx \<tau>s \<Gamma>2"
      using matches_split2 v_sem_take.prems
      by blast+
    
    have "\<Xi>\<^sub>d, \<Xi> \<turnstile> VRecord fs :v instantiate \<tau>s (TRecord ts s)"
    ...
    moreover then have all_inst_ts:
      "\<Xi>\<^sub>d, \<Xi> \<turnstile>* fs :vr map (\<lambda>(n, t, b). (n, instantiate \<tau>s t, b)) ts"
      "distinct (map fst ts)"
      by (fastforce)+
    moreover then have "\<And>t' b'. distinct (map fst (ts[f := (n, t', b')]))"
      by (simp add: map_fst_update typing_elims)
    ultimately have "\<Xi>\<^sub>d, \<Xi> \<turnstile> VRecord fs :v TRecord (map (\<lambda>(n, t, b). (n, instantiate \<tau>s t, b)) (ts[f := (n, t, taken)])) s"
  ...
  qed simp+
next
  case (v_sem_put \<xi>\<^sub>d \<xi> \<gamma> x_spec fs ea_spec ea' f)
  ...
  proof (case_tac e; clarsimp)
    ...
    then obtain \<Gamma>1 \<Gamma>2 ts s n t taken k
      where typingelims:
        ...
        "\<Xi>\<^sub>d, \<Xi>, K, \<Gamma>2 \<turnstile> ea : t"
      using v_sem_put by blast
    
    have IHresults:
      "\<Xi>\<^sub>d, \<Xi> \<turnstile> VRecord fs :v instantiate \<tau>s (TRecord ts s)"
      "\<Xi>\<^sub>d, \<Xi> \<turnstile> ea' :v instantiate \<tau>s t"
    ...
    then obtain ts' s'
      where instvrecordelims:
        ...
        "\<Xi>\<^sub>d, \<Xi> \<turnstile>* fs :vr ts'"
        "distinct (map fst ts')"
      by blast

    show "\<Xi>\<^sub>d, \<Xi> \<turnstile> VRecord (fs[f := ea']) :v instantiate \<tau>s \<tau>"
...
next case (v_sem_app \<xi>\<^sub>d \<xi> \<gamma> x ea ts y a r e \<tau>s K \<tau> \<Gamma>)
  ...
  obtain \<Gamma>1 \<Gamma>2 t targ where app_elims:
    "K \<turnstile> \<Gamma> \<leadsto> \<Gamma>1 | \<Gamma>2"
    "t=  TFun targ \<tau> \<or>  t = TDFun targ \<tau>"
    "\<Xi>\<^sub>d, \<Xi>, K, \<Gamma>1 \<turnstile> efun : t"
    "\<Xi>\<^sub>d, \<Xi>, K, \<Gamma>2 \<turnstile> earg : targ"
    using v_sem_app e_def by blast 

  have vfun_ty: "\<Xi>\<^sub>d, \<Xi> \<turnstile> VFunction ea ts :v instantiate \<tau>s (TFun targ \<tau>)"
    using app_elims e_def v_sem_app matches_split 
    by (smt instantiate.simps(6) type.distinct(69) value_sem.v_t_funE value_sem_axioms)
   (* check this proof *)

  have varg_ty: "\<Xi>\<^sub>d, \<Xi> \<turnstile> a :v instantiate \<tau>s targ"
    by (metis app_elims e_def v_sem_app matches_split)
    

  obtain Kfun t u where vfun_ty_elims: "\<Xi>\<^sub>d, \<Xi>, Kfun, [Some t] \<turnstile> ea : u"
  ...
  have vres_ty_sub: "\<Xi>\<^sub>d, \<Xi> \<turnstile> r :v instantiate ts u"
...
next case (v_sem_abs_app \<xi>\<^sub>d \<xi> \<gamma> x f ts y a r)
  obtain efun earg where e_def: "e = App efun earg"
      "x = specialise \<tau>s efun"
      "y = specialise \<tau>s earg"
  ...
  obtain \<Gamma>1 \<Gamma>2 targ where app_elims:
    "K \<turnstile> \<Gamma> \<leadsto> \<Gamma>1 | \<Gamma>2"
    "\<Xi>\<^sub>d, \<Xi>, K, \<Gamma>1 \<turnstile> efun : TFun targ \<tau>"
    "\<Xi>\<^sub>d, \<Xi>, K, \<Gamma>2 \<turnstile> earg : targ"
    using v_sem_abs_app e_def  

  have vafun_ty: "\<Xi>\<^sub>d, \<Xi> \<turnstile> VAFunction f ts :v instantiate \<tau>s (TFun targ \<tau>)"
...
next case v_sem_all_cons  then show ?case by ( case_tac es, simp_all
                                             , fastforce simp: vval_typing_all_def
                                                         dest: matches_split)
next
  case (v_sem_promote \<Xi> \<xi>\<^sub>d \<xi> \<gamma> ea ea' t)
  then show ?case
...
function monoexpr :: "('f, 'g) expr \<Rightarrow> ('f \<times> type list) expr" where
...
fun monoval :: "('f, 'g, 'a, 'b) vval \<Rightarrow> ('f \<times> type list, 'a, 'b) vval"
...
definition monoprog :: "('f, 'g, 'a, 'b) vabsfuns \<Rightarrow> (('f \<times> type list), 'a, 'b) vabsfuns \<Rightarrow> bool"
...
lemma v_sem_prim': "\<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile>* as \<Down> as' \<Longrightarrow> eval_prim p as' = r \<Longrightarrow> \<xi>\<^sub>d , \<xi> , \<gamma> \<turnstile> Prim p as \<Down> r"
...
and     "monoprog \<xi> \<xi>'"
shows   "\<xi>\<^sub>d, \<xi>, \<gamma> \<turnstile> e \<Down> e' \<Longrightarrow>  \<Xi>, [], \<Gamma> \<turnstile> e : \<tau>    \<Longrightarrow> \<xi>', map monoval \<gamma> \<turnstile> monoexpr e \<Down> monoval e'"
 and     "\<xi>\<^sub>d, \<xi>, \<gamma> \<turnstile>* es \<Down> es' \<Longrightarrow> \<Xi>, [], \<Gamma> \<turnstile>* es : \<tau>s \<Longrightarrow> \<xi>', map monoval \<gamma> \<turnstile>* map monoexpr es \<Down> map monoval es'"
