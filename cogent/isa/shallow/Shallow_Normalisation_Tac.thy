...
fun anormal_prepare_FO_conv ctxt ct = let
  ...
  fun fun_app_eqn (f, x) = let
        ...
        val at = Thm.instantiate_cterm ([(aT, aT'), (bT, bT')], []) avar
        val bt = Thm.instantiate_cterm ([(aT, aT')], []) bvar
        in Thm.instantiate ([(aT, aT'), (bT, bT')], [(at, f), (bt, x)]) rule end
  fun expand_conv ct =
...
fun anormal_proc extra_rules = fn ctxt => fn t =>
  if in_continuation t then NONE else let
    in case Net.match_term (anormal_proc_net extra_rules) t of
          [] => NONE
        | (eqn::_) => SOME eqn end

fun anormal_simproc extra_rules = mk_simproc "cogent_anormal" [@{cpat "_ $ _"}] (anormal_proc extra_rules)
...
fun normalisation_tac ctxt
  ...
  fun anormal_conv def = def
      ...
      |> Conv.fconv_rule (Simplifier.rewrite (put_simpset HOL_basic_ss ctxt addsimprocs 
              [mk_simproc "ucast_simproc" [@{cpat "ucast (numeral _)"}, @{cpat "ucast zero_class.zero"}, @{cpat "ucast one_class.one"}]
                  (fn _ => conv_to_simproc (fn ct => Simplifier.rewrite ctxt (* NB: outer ctxt to avoid recursion *) ct) ctxt)]))
