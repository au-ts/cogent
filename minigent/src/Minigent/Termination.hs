      ...
      case getType f genvs of 
        ...
        Just ty ->
          let (funName, b) = fst $ runFresh unifVars (init' f x e ty)
              ...
              templates = [] --fill funName e M.empty 0 template template
...
fillTemplates :: FunName -> Expr -> Template -> Env -> Int -> ([(Template, Expr)], Env)
fillTemplates funName exp tem env n = 
  case exp of
    PrimOp op (e:es) -> case es of 
      [x] -> 
        let (res1, env1) = fillTemplates funName e tem env n
            (res2, env2) = fillTemplates funName x tem env n
        in (res1 ++ res2, env2)
      ...
      case tem of 
        RecordAST mv xs -> 
          case find (\(f', mv', t') -> f == f') xs of 
            Nothing -> ([], env)
            Just (f', mv', t') -> 
              let (res1, env1) = fillTemplates funName e2 t' env (n+1)
              in case res1 of 
                [] -> ([], env1)
                _ -> (map (\(x, y) -> (RecordAST mv [x], y)) res1, env1)
        RecursiveRecordAST rp mv xs -> 
          case find (\(f', mv', t') -> f == f') xs of 
            ...
            Just (f', mv', t') -> 
              ...
              in case res1 of 
                [] -> ([], env1)
                _ -> (map (\(x, y) -> (RecursiveRecordAST rp mv [x], y)) res1, env1)
        _ -> ([], env) -- Error, as we should only be seeing a record when taking ?
    Member e f ->
...
--               case envExists (Var v) of 
--                 False -> ([], env)
--                 True ->
--                 -- if OK, then add v1 into the env 
--                   let env1 = envAdd v1 env 
--                   in 
--                   -- search for c in the template
--                   case find (\(f', mv', t') -> f' == c) of 
--                     Nothing -> ([], env1)
--                     Just (f', mv', t') -> 
--                       let (res1, env2) = fillTemplates funName e2 t' env1 (n+1)
--                       in (map (\x -> VariantAST mv [x]) res1, env2)
--             -- Should be seeing a variant; if not, problem.
--             _ -> ([], env)
    Case e1 c v1 e2 v2 e3 -> 
      case e1 of 
        Var v -> 
          case tem of 
            VariantAST  mv xs -> 
              ...
              case envExists (Var v) env of 
                False -> ([], env)
                True -> 
                  -- if OK, add v1 into the env.
                  let env1 = envAdd (Var v1) env
                  in 
                    -- search for c in the template
                    case find (\(f', mv', t') -> f' == c) xs of 
                      Nothing -> ([], env1)
                        -- check e3???
                        -- let (res2, env3) = fillTemplates funName e3 t' env1 (n+1)
                        -- in case res2 of 
                        --   [] -> ([], env3)
                        --   _ -> (map (\(x, y) -> (VariantAST mv [x], y)) res2, env3)
                      Just (f', mv', t') -> 
                        let (res1, env2) = fillTemplates funName e2 t' env1 (n+1)
                            (res2, env3) = fillTemplates funName e3 t' env1 (n+1)
                            res3 = res1 ++ res2
                        in case res3 of 
                          [] -> ([], env3)
                          _ -> (map (\x -> VariantAST mv [x]) res3, env3)
            ...
            RecordAST mv xs -> 
              case find (\(f', mv', t') -> f == f') xs of 
                ...
                Just (f', mv', t') ->
                  ...
                  case t' of 
                    VariantAST mv1 xs1 -> 
                      -- check if the ConName matches the field.
                      case find (\(f'', mv'', t'') -> f == c) xs1 of
                        Nothing -> -- check e3.
                          -- set v2 = member e4 f in the env
                          let (res1, env1) = fillTemplates funName e3 t' env (n+2)
                          -- wrap back up TWO LAYERS
                          in case res1 of 
                            [] -> ([], env1)
                            _ -> (map (\(x, y) -> (RecordAST mv (VariantAST mv1 [x], y))) res1, env1)
                        Just (f'', mv'', t'') -> -- take e2, and then check e3 as well.
                          let (res1, env1) = fillTemplates funName e2 t' env (n+2)
                              (res2, env2) = fillTemplates funName e3 t' env1 (n+2)
                              res3 = res1 ++ res2
                          in
                            -- wrap back up 2 layers.
                            case res3 of 
                              [] -> ([], env2)
                              _ -> (map (\(x, y) -> (RecordAST mv (VariantAST mv1 [x]), y)) res3, env2)
                  ...
                  case t' of 
                    VariantAST mv1 xs1 -> 
                      ...
                      case find (\(f'', mv'', t'') -> f == c) xs1 of
                        Nothing -> -- check e3.
                          -- set v2 = member e4 f in the env
                          let (res1, env1) = fillTemplates funName t' e3 env (n+2)
                          -- wrap back up TWO LAYERS
                          in (map (\(x, y) -> (RecursiveRecordAST rp mv (VariantAST mv1 [x]), y)) res1, env1)
                        Just (f'', mv'', t'') -> -- take e2, and then check e3 as well.
                          let (res1, env1) = fillTemplates funName t' e2 env (n+2)
                              (res2, env2) = fillTemplates funName t' e3 env1 (n+2)
                              res3 = res1 ++ res2
                          in
                            -- wrap back up 2 layers.
                            case res3 of 
                              [] -> ([], env2)
                              _ -> (map (\(x, y) -> (RecursiveRecordAST rp mv (VariantAST mv1 [x]), y)) res3, env2)
                    -- Error, we should always be seeing a variant template here.
                    _ -> ([], env)
              ...
              case envExists (Var v) of 
                ...
                -- if OK, then add v1 into the env 
                  ...
                  case find (\(f', mv', t') -> f' == c) of 
                    ...
                    Just (f', mv', t') -> 
                      let (res1, env2) = fillTemplates funName e2 t' env1 (n+1)
                      in (map (\(x, y) -> (VariantAST mv [x], y)) res1, env2)
            -- Should be seeing a variant; if not, problem.
            _ -> ([], env)
        Member e3 f -> 
          case tem of 
            VariantAST mv xs -> 
              -- check that e3 exists in the env  (from before)
              case envExists e3 env of 
                False -> ([], env)
                True -> 
                  -- check if there exists A variant of the relevant field
                  case find (\(f', mv', t') -> f' == f) xs of 
                    Nothing -> ([], env)
                    Just (f', mv', t') -> 
                      let (res1, env1) = fillTemplates e2 t' env (n+1)
                          env' = M.insert 
                      in 
                        -- wrap the input calls. 
                        (map (\(x, y) -> (VariantAST mv [x], y)) res1, env')
            -- Should be seeing a variant; if not, problem.
            _ -> ([], env)
        -- case and esac only cover var and member expressions
        _ -> ([], env)
